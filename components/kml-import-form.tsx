  ...
  return <form action={formAction} className="flight-form kml-wizard" onChangeCapture={markDirty} onSubmitCapture={beginSubmit}>
    ...
    {analysis&&analysis.points.length>=2?<>
      ...
      <div className="form-grid secondary-entry-grid">
        <label>Registration <span className="field-hint" aria-hidden="true">Required</span><select name="registration" required value={registration} onChange={event=>{setRegistration(event.target.value);setBalloonOperation("")}}><option value="">Select</option>{aircraft.map(item=><option key={item.registration}>{item.registration}</option>)}</select>{analysis.registration?<small>Suggested: {analysis.registration}</small>:null}</label>
        ...
        {selectedBalloon?<label>Balloon operation <span className="field-hint" aria-hidden="true">Required</span><select name="balloonOperation" value={balloonOperation} onChange={event=>setBalloonOperation(event.target.value)} required><option value="">Select free / tethered</option><option value="FREE">Free flight</option><option value="TETHERED">Tethered flight</option></select><small>Required BFCL evidence. GPS cannot determine whether the operation was free or tethered.</small></label>:<input type="hidden" name="balloonOperation" value=""/>}
      ...
      <div className="flight-review-list">{parts.map((part,index)=>{
        ...
        return <article className={`flight-review-card ${review.reviewed?"confirmed":""}${credible?"":" invalid-flight"}`} key={`${cuts.join("-")}-${index}`} tabIndex={-1}>
          ...
          <div className="form-grid review-grid">
            <label>Date <span className="field-hint" aria-hidden="true">Required</span><input name={`part_${index}_date`} type="date" required value={review.date} onChange={event=>updateReview(index,{date:event.target.value,reviewed:false})}/></label>
