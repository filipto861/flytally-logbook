...
export function PushNotificationSettings(){
  ...
  return <section className="panel u33-preference-card push-settings">
    ...
    {state==="enabled"?<button className="secondary-button" type="button" disabled={busy} aria-busy={busy||undefined} data-loading={busy?"true":undefined} onClick={()=>void disable()}>{busy?"Disabling…":"Disable on this device"}</button>:state==="install-required"?<div className="push-install-note"><strong>iPhone / iPad</strong><small>Use Safari → Share → Add to Home Screen. Open the installed FlyTally app, then return here to enable notifications.</small></div>:state==="denied"?<p className="muted">Open this site's notification permissions in your browser and change Notifications to Allow, then reload FlyTally.</p>:state==="unsupported"?<p className="muted">This browser does not expose the Web Push APIs required by FlyTally.</p>:<button className="primary-button" type="button" disabled={busy} aria-busy={busy||undefined} data-loading={busy?"true":undefined} onClick={()=>void enable()}>{busy?"Enabling…":"Enable on this device"}</button>}
