...
export function installResponsiveMap(map:L.Map,target:HTMLElement){
  ...
  if(window.matchMedia("(max-width: 820px), (pointer: coarse)").matches){
    ...
    const button=document.createElement("button");button.type="button";button.className="map-touch-toggle";button.textContent="Povolit pohyb mapy";button.setAttribute("aria-pressed","false");
    L.DomEvent.disableClickPropagation(button);L.DomEvent.disableScrollPropagation(button);
    button.addEventListener("click",event=>{event.preventDefault();const enabled=button.getAttribute("aria-pressed")==="true";if(enabled){map.dragging.disable();map.touchZoom.disable();button.textContent="Povolit pohyb mapy";button.setAttribute("aria-pressed","false")}else{map.dragging.enable();map.touchZoom.enable();button.textContent="Ukončit pohyb mapy";button.setAttribute("aria-pressed","true")}});
