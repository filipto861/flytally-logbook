...
export function FlightTrackPlayer({tracks}:{tracks:MapTrack[]}) {
  ...
  useEffect(()=>{if(!target.current||!samples.length)return;const map=L.map(target.current,{preferCanvas:true,zoomControl:true});mapRef.current=map;L.tileLayer("https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png",{maxZoom:20,attribution:"&copy; OpenStreetMap contributors &copy; CARTO"}).addTo(map);const bounds=L.latLngBounds([]);tracks.forEach((track,i)=>{const ll=track.points.map(p=>L.latLng(p.lat,p.lon)),color=i%2?"#38bdf8":"#34d399";ll.forEach(p=>bounds.extend(p));L.polyline(ll,{color,weight:3,opacity:.92,renderer:L.canvas()}).addTo(map);const depGap=track.departurePoint?leg(track.departurePoint,track.points[0]):0,arrGap=track.arrivalPoint?leg(track.points.at(-1)!,track.arrivalPoint):0;if(track.departurePoint&&depGap>.35){const c=[L.latLng(track.departurePoint.lat,track.departurePoint.lon),ll[0]];c.forEach(p=>bounds.extend(p));L.polyline(c,{color,weight:2,dashArray:"7 7",opacity:.85}).addTo(map)}if(track.arrivalPoint&&arrGap>.35){const c=[ll.at(-1)!,L.latLng(track.arrivalPoint.lat,track.arrivalPoint.lon)];c.forEach(p=>bounds.extend(p));L.polyline(c,{color,weight:2,dashArray:"7 7",opacity:.85}).addTo(map)}});const icon=L.divIcon({className:"aircraft-marker",html:'<span>✈</span>',iconSize:[34,34],iconAnchor:[17,17]});markerRef.current=L.marker([samples[0].lat,samples[0].lon],{icon,zIndexOffset:1000}).addTo(map);map.fitBounds(bounds,{padding:[25,25],maxZoom:13});return()=>{map.remove();mapRef.current=null;markerRef.current=null}},[samples,tracks]);
