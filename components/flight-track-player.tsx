...
export function FlightTrackPlayer({tracks}:{tracks:MapTrack[]}) {
  ...
  useEffect(()=>{if(!target.current||!samples.length)return;const map=L.map(target.current,{preferCanvas:true,zoomControl:true});mapRef.current=map;L.tileLayer("https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png",{maxZoom:20,attribution:"&copy; OpenStreetMap contributors &copy; CARTO"}).addTo(map);const bounds=L.latLngBounds([]);tracks.forEach((track,i)=>{const ll=track.points.map(p=>L.latLng(p.lat,p.lon)),color=i%2?"#38bdf8":"#34d399";ll.forEach(p=>bounds.extend(p));L.polyline(ll,{color,weight:3,opacity:.92,renderer:L.canvas()}).addTo(map);const depGap=track.departurePoint?leg(track.departurePoint,track.points[0]):0,arrGap=track.arrivalPoint?leg(track.points.at(-1)!,track.arrivalPoint):0;if(track.departurePoint&&depGap>.35&&depGap<150){const c=[L.latLng(track.departurePoint.lat,track.departurePoint.lon),ll[0]];c.forEach(p=>bounds.extend(p));L.polyline(c,{color,weight:2,dashArray:"7 7",opacity:.85}).addTo(map)}if(track.arrivalPoint&&arrGap>.35&&arrGap<150){const c=[ll.at(-1)!,L.latLng(track.arrivalPoint.lat,track.arrivalPoint.lon)];c.forEach(p=>bounds.extend(p));L.polyline(c,{color,weight:2,dashArray:"7 7",opacity:.85}).addTo(map)}});const icon=L.divIcon({className:"aircraft-marker",html:'<span>✈</span>',iconSize:[34,34],iconAnchor:[17,17]});markerRef.current=L.marker([samples[0].lat,samples[0].lon],{icon,zIndexOffset:1000}).addTo(map);map.fitBounds(bounds,{padding:[25,25],maxZoom:13});return()=>{map.remove();mapRef.current=null;markerRef.current=null}},[samples,tracks]);
  ...
  return <section className="track-player-suite">
    ...
    <div className="panel track-profile"><div className="section-heading"><div><p className="eyebrow">GPS PŘEHRÁVAČ</p><h2>Výška, rychlost a poloha</h2></div><div className="track-stats"><span>{samples.at(-1)?.distance.toFixed(1)} km</span><span>{samples.length} bodů</span><span>max {Math.round(Math.max(...alts)*3.28084)} ft</span><span>max {Math.round(Math.max(...speeds))} km/h</span></div></div>
      <div className="combined-profile-chart"><div className="profile-legend"><span><i className="alt-line"/>Výška</span><span><i className="speed-line"/>Rychlost</span><small>Výška a rychlost mají vlastní měřítko</small></div><svg viewBox={`0 0 ${w} ${h}`} preserveAspectRatio="none"><polyline points={altLine} fill="none" stroke="#38bdf8" strokeWidth="3" vectorEffect="non-scaling-stroke"/><polyline points={speedLine} fill="none" stroke="#34d399" strokeWidth="3" vectorEffect="non-scaling-stroke"/><line x1={x} x2={x} y1="0" y2={h} stroke="#f8fafc" strokeWidth="2" vectorEffect="non-scaling-stroke"/></svg></div>
