...
export function FlightTrackPlayer({tracks}:{tracks:MapTrack[]}) {
  ...
  useEffect(()=>{if(!target.current||!samples.length)return;const map=L.map(target.current,{preferCanvas:true,zoomControl:true});mapRef.current=map;addFlyTallyBasemap(map);const renderer=L.canvas({padding:.3}),bounds=L.latLngBounds([]);tracks.forEach((track,i)=>{const ll=track.points.map(p=>L.latLng(p.lat,p.lon)),color=i%2?palette.route:palette.routeAlt;ll.forEach(p=>bounds.extend(p));L.polyline(ll,{color,weight:3,opacity:.92,renderer}).addTo(map);const depGap=track.departurePoint?leg(track.departurePoint,track.points[0]):0,arrGap=track.arrivalPoint?leg(track.points.at(-1)!,track.arrivalPoint):0;if(track.departurePoint&&depGap>.35){const c=[L.latLng(track.departurePoint.lat,track.departurePoint.lon),ll[0]];c.forEach(p=>bounds.extend(p));L.polyline(c,{color,weight:2,dashArray:"7 7",opacity:.85,renderer}).addTo(map)}if(track.arrivalPoint&&arrGap>.35){const c=[ll.at(-1)!,L.latLng(track.arrivalPoint.lat,track.arrivalPoint.lon)];c.forEach(p=>bounds.extend(p));L.polyline(c,{color,weight:2,dashArray:"7 7",opacity:.85,renderer}).addTo(map)}});const icon=L.divIcon({className:"aircraft-marker",html:'<span>✈</span>',iconSize:[34,34],iconAnchor:[17,17]});markerRef.current=L.marker([samples[0].lat,samples[0].lon],{icon,zIndexOffset:1000}).addTo(map);map.fitBounds(bounds,{padding:[25,25],maxZoom:13});const cleanupResponsive=installResponsiveMap(map,target.current);return()=>{cleanupResponsive();map.remove();mapRef.current=null;markerRef.current=null}},[samples,tracks,theme]);
