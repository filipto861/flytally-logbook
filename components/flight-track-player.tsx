...
export function FlightTrackPlayer({tracks}:{tracks:MapTrack[]}) {
  ...
  useEffect(()=>{if(!target.current||!samples.length)return;const map=L.map(target.current,{preferCanvas:true,zoomControl:true});mapRef.current=map;L.tileLayer("https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png",{maxZoom:20,attribution:"&copy; OpenStreetMap contributors &copy; CARTO"}).addTo(map);const bounds=L.latLngBounds([]);tracks.forEach((track,i)=>{const ll=track.points.map(p=>L.latLng(p.lat,p.lon));ll.forEach(p=>bounds.extend(p));L.polyline(ll,{color:i%2?"#38bdf8":"#34d399",weight:3,opacity:.92,renderer:L.canvas()}).addTo(map)});const icon=L.divIcon({className:"aircraft-marker",html:'<span>✈</span>',iconSize:[34,34],iconAnchor:[17,17]});markerRef.current=L.marker([samples[0].lat,samples[0].lon],{icon,zIndexOffset:1000}).addTo(map);map.fitBounds(bounds,{padding:[25,25],maxZoom:13});return()=>{map.remove();mapRef.current=null;markerRef.current=null}},[samples,tracks]);
