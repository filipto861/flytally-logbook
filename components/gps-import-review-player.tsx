...
export function GpsImportReviewPlayer({track,events=[]}:{track:MapTrack;events?:ImportReviewEvent[]}){
  ...
  useEffect(()=>{if(!target.current||samples.length<2)return;const map=L.map(target.current,{preferCanvas:true,zoomControl:true});mapRef.current=map;L.tileLayer("https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png",{maxZoom:20,attribution:"&copy; OpenStreetMap contributors &copy; CARTO"}).addTo(map);const renderer=L.canvas({padding:.3}),ll=points.map(point=>L.latLng(point.lat,point.lon)),bounds=L.latLngBounds(ll);L.polyline(ll,{color:"#38bdf8",weight:3,opacity:.92,renderer}).addTo(map);const icon=L.divIcon({className:"aircraft-marker",html:'<span>✈</span>',iconSize:[34,34],iconAnchor:[17,17]});markerRef.current=L.marker([samples[0].lat,samples[0].lon],{icon,zIndexOffset:1000}).addTo(map);map.fitBounds(bounds,{padding:[24,24],maxZoom:13});const cleanup=installResponsiveMap(map,target.current);return()=>{cleanup();map.remove();mapRef.current=null;markerRef.current=null}},[points,samples]);
  ...
  return <section className="track-player-suite import-review-player">
    ...
    <div className="panel track-profile">
      <div className="section-heading"><div><p className="eyebrow">VISUAL GPS REVIEW</p><h2>Altitude, speed and detected events</h2><p className="muted">Play or scrub the track. Select any event marker to move the aircraft to that position.</p></div><div className="track-stats"><span>{samples.at(-1)?.distance.toFixed(1)} km</span><span>{samples.length} points</span><span>max {Math.round(maxAlt*3.28084)} ft</span><span>max {Math.round(maxSpeed)} km/h</span></div></div>
      <div className="combined-profile-chart import-profile-chart"><div className="profile-legend"><span><i className="alt-line"/>Altitude</span><span><i className="speed-line"/>Speed</span><small>Markers are advisory GPS detections.</small></div><div className="profile-chart-stage"><svg viewBox={`0 0 ${width} ${height}`} preserveAspectRatio="none"><polyline points={altLine} fill="none" stroke="#38bdf8" strokeWidth="3" vectorEffect="non-scaling-stroke"/><polyline points={speedLine} fill="none" stroke="#34d399" strokeWidth="3" vectorEffect="non-scaling-stroke"/><line x1={cursorX} x2={cursorX} y1="0" y2={height} stroke="#f8fafc" strokeWidth="2" vectorEffect="non-scaling-stroke"/></svg><div className="profile-event-layer" aria-label="Detected GPS events">{cleanEvents.map((event,eventIndex)=><button type="button" key={`${event.kind}-${event.position}-${eventIndex}`} className={`profile-event-marker ${event.kind}`} style={{left:`${event.position*100}%`}} onClick={()=>seek(event.position)} title={event.detail?`${event.label} · ${event.detail}`:event.label}><span>{event.label}</span></button>)}</div></div></div>
