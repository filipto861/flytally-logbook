...
function RateTimeline({aircraft,rates,saveAction,deleteAction}:{aircraft:Row;rates:Row[];saveAction:Action;deleteAction:Action}){
  ...
  return <section className="aircraft-rates"><div className="modal-section-heading"><div><p className="eyebrow">RATES</p><h3>Rate history</h3></div><span>{history.length}</span></div>
    ...
    <div className="rate-history">{history.map((rate,index)=><div className={`rate-history-row${index===0?" newest":""}`} key={t(rate.id)}><span><small>Valid from</small><b>{t(rate.valid_from)||"No date"}</b></span><span><small>Rate</small><b>{Number(rate.price_per_hour||0).toLocaleString("en-GB")} CZK/h</b></span><span><small>Dry</small><b>{Number(rate.dry_price_per_hour||0)?`${Number(rate.dry_price_per_hour).toLocaleString("en-GB")} CZK/h`:"—"}</b></span><span><small>Source</small><b>{t(rate.source)||"—"}</b></span><form action={deleteAction}><input type="hidden" name="id" value={t(rate.id)}/><button className="icon-danger">Delete</button></form></div>)}{!history.length?<p className="empty-state">No rate history.</p>:null}</div>
  </section>;
}
