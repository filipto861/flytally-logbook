...
export function FlightTrash({flights,restoreAction,timeZone}:{flights:DeletedFlight[];restoreAction:Action;timeZone:string}){
  ...
  return <section className="panel flight-trash"><header><div><p className="eyebrow">RECENTLY DELETED</p><h2>Flight recycle bin</h2></div><span>{flights.length}</span></header>
    ...
    {flights.length?<div className="trash-list">{flights.map(flight=><article key={flight.id}><div><strong>{flight.registration} · {formatDateOnly(flight.date)}</strong><span>{flight.departure||"—"} → {flight.arrival||"—"}</span><small>Deleted {formatLocalDateTime(flight.deletedAt,timeZone)} · {flight.trackCount} GPS tracks</small></div><button type="button" className="secondary-button" disabled={pending!==null} aria-busy={pending===flight.id||undefined} data-loading={pending===flight.id?"true":undefined} onClick={()=>restore(flight.id)}>{pending===flight.id?"Restoring…":"Restore flight"}</button></article>)}</div>:<p className="empty-state">No deleted flights.</p>}
  </section>;
}
