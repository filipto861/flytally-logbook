...
export function Sidebar({role="user"}:{role?:"admin"|"user"}){
  ...
  return <aside className={`sidebar${collapsed?" collapsed":""}${mobile?" mobile-open":""}`}>
    <div className="sidebar-brand"><span className="brand-symbol"><img src="/icon.svg" alt="" /></span><div><p className="eyebrow">PILOT</p><h2>Logbook</h2></div><button className="sidebar-toggle" type="button" onClick={toggle} aria-label="Sbalit navigaci">{collapsed?"›":"‹"}</button><button className="mobile-toggle" type="button" onClick={()=>setMobile(!mobile)} aria-label="Otevřít navigaci">☰</button></div>
