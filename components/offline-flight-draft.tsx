...
import { FLIGHT_DRAFT_STORAGE_KEY,flightDraftSummary,parseFlightDraft,type StoredFlightDraft } from "@/lib/offline-flight-draft";
...
export function OfflineFlightDraft(){
  const[draft,setDraft]=useState<StoredFlightDraft|null>(null),[online,setOnline]=useState(true),[message,setMessage]=useState("");
  const values=draft?.values??{};

  useEffect(()=>{
    setOnline(navigator.onLine);
    setDraft(parseFlightDraft(localStorage.getItem(FLIGHT_DRAFT_STORAGE_KEY)));
  ...
  const update=(key:string,value:string)=>{
    setDraft(current=>({version:1,id:current?.id||draftId(),updatedAt:new Date().toISOString(),values:{...(current?.values??{}),[key]:value}}));
    setMessage("");
  };
  const save=()=>{
    const next=draft??{version:1 as const,id:draftId(),updatedAt:new Date().toISOString(),values:{}};
    const stored={...next,updatedAt:new Date().toISOString()};
    localStorage.setItem(FLIGHT_DRAFT_STORAGE_KEY,JSON.stringify(stored));setDraft(stored);setMessage("Saved on this device.");
  };
  const clear=()=>{localStorage.removeItem(FLIGHT_DRAFT_STORAGE_KEY);setDraft(null);setMessage("Local draft cleared.")};
  ...
  return <section className="offline-draft-card">
    <div className="offline-draft-heading"><div><p className="eyebrow">LOCAL DRAFT</p><h2>{draft?flightDraftSummary(draft):"New offline flight draft"}</h2><p className="muted">Stored only in this browser until you open Add Flight and save it to FlyTally.</p></div><span className={`network-pill ${online?"online":"offline"}`}>{online?"Online":"Offline"}</span></div>
    <div className="form-grid essential-grid offline-draft-grid">
      <label>Date<input type="date" value={values.date||new Date().toISOString().slice(0,10)} onChange={event=>update("date",event.target.value)}/></label>
    ...
    <div className="offline-draft-actions"><button type="button" className="primary-button" onClick={save}>Save locally</button>{draft?<button type="button" className="secondary-button" onClick={clear}>Clear draft</button>:null}{online?<Link className="secondary-link" href="/flights/new">Continue in Add Flight</Link>:null}</div>
