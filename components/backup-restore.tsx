...
const labels:Record<string,string>={flights:"Flights",aircraft:"Aircraft",rates:"Rates",airports:"Custom airports",expiries:"Expiries",flight_tracks:"GPS tracks"};
...
export function BackupRestore({action}:{action:Action}){
  ...
  return <section className="panel backup-restore"><div><p className="eyebrow">SAFE RESTORE</p><h2>Restore missing data</h2></div>
    ...
    {preview?<div className="restore-preview"><header><div><strong>✓ Valid backup</strong><small>Export: {preview.exportedAt?new Date(preview.exportedAt).toLocaleString("en-GB"):"date unavailable"}</small></div><span>SHA-256 verified</span></header><div className="restore-grid">{Object.keys(preview.source).map(key=><article key={key}><strong>{labels[key]||key}</strong><span><b>{preview.add[key]||0}</b> add</span><small>{preview.skip[key]||0} existing</small></article>)}</div><div className="restore-confirm"><label>Type RESTORE to confirm<input value={confirm} onChange={event=>setConfirm(event.target.value)} autoComplete="off"/></label><button type="button" className="primary-button" disabled={pending||confirm.trim().toUpperCase()!=="RESTORE"} onClick={()=>run("restore")}>{pending?"Restoring…":"2. Restore missing data"}</button></div></div>:null}
  </section>;
}
