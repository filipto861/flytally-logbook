...
export async function AircraftShareInbox(){
  ...
  return <>
    ...
    {outgoing.length?<section className="panel"><div className="section-heading"><div><p className="eyebrow">SENT</p><h2>Aircraft profiles waiting for import</h2><p className="muted">These are copies you sent to Connections. Your own aircraft remains unchanged.</p></div><span>{outgoing.length}</span></div><div className="connection-list">{outgoing.map(row=>{const p=profileFrom(row.snapshot_data);return <article className="connection-card" key={`aircraft-sent-${text(row.id)}`}><div className="pilot-avatar" aria-hidden="true">✈</div><div className="connection-identity"><strong>{p.registration||"Aircraft profile"}</strong><span>Sent to {text(row.display_name)||"Pilot"}</span><small>{p.model||"Profile"} · waiting for import decision</small></div><form action={cancelAircraftProfileShare}><input type="hidden" name="share_id" value={text(row.id)}/><PendingActionButton className="secondary-button" pendingLabel="Cancelling…">Cancel share</PendingActionButton></form></article>})}</div></section>:null}
  </>;
}
