...
export function DashboardDetails({data}:{data:DashboardData}){
  ...
  return <section className="panel details-panel">
    ...
    {open?<div className="detail-stats">
      <div className="mini-metrics"><div><span>Air time</span><b>{formatDuration(data.airMinutes)}</b></div><div><span>PIC</span><b>{formatDuration(data.picMinutes)}</b></div><div><span>Co-pilot</span><b>{formatDuration(data.copilotMinutes)}</b></div><div><span>DUAL</span><b>{formatDuration(data.dualMinutes)}</b></div><div><span>Instructor</span><b>{formatDuration(data.instructorMinutes)}</b></div><div><span>Night</span><b>{formatDuration(data.nightMinutes)}</b></div><div><span>IFR</span><b>{formatDuration(data.ifrMinutes)}</b></div><div><span>Day / night landings</span><b>{data.dayLandings} / {data.nightLandings}</b></div><div><span>Cost</span><b>{Math.round(data.cost).toLocaleString("en-GB")} CZK</b></div></div>
