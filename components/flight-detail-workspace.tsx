...
export function FlightDetailWorkspace({ overview, gps, logbook, gpsCount = 0, initialTab = "overview" }: { overview: ReactNode; gps: ReactNode; logbook: ReactNode; gpsCount?: number; initialTab?: Tab }) {
  ...
  useEffect(() => {
    const raw = sessionStorage.getItem(POST_SAVE_REVIEW_KEY);
    if (!raw) return;
    sessionStorage.removeItem(POST_SAVE_REVIEW_KEY);
    const savedAt = Number(raw);
    const age = Date.now() - savedAt;
    if (initialTab === "overview" && Number.isFinite(savedAt) && age >= 0 && age <= POST_SAVE_REVIEW_MAX_AGE_MS) {
      setTab("logbook");
      setPostSaveReview(true);
    }
  }, [initialTab]);

  const selectTab = (value: Tab) => {
    setTab(value);
    setPostSaveReview(false);
  };
  const item = (value: Tab, label: string, badge?: number) => (
    <button type="button" role="tab" id={`flight-tab-${value}`} aria-controls={`flight-panel-${value}`} className={tab === value ? "active" : ""} aria-selected={tab === value} onClick={() => selectTab(value)}>
      <span>{label}</span>{badge !== undefined ? <b>{badge}</b> : null}
    </button>
  );

  return <section className="flight-detail-workspace">
    <nav className="detail-tabs" role="tablist" aria-label="Flight detail sections">
      {item("overview", "Overview")}{item("gps", "GPS track", gpsCount)}{item("logbook", "Logbook data")}
    </nav>
    ...
    <div className="flight-detail-secondary-actions"><Link href={`${pathname}/share`} aria-label="Share this flight">↗ Share flight</Link></div>
  </section>;
}
