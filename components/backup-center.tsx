...
export function BackupCenter({backups,createAction,restoreAction}:{backups:StoredBackup[];createAction:()=>Promise<void>;restoreAction:RestoreAction}){
  ...
  return <section className="panel backup-center"><header><div><p className="eyebrow">BACKUP CENTER</p><h2>Stored account backups</h2></div><form action={createAction}><button className="primary-button">Create backup now</button></form></header>
    ...
    {backups.length?<><div className="stored-backup-list">{(showAll?backups:backups.slice(0,6)).map(backup=><article key={backup.id} className={selected===backup.id?"selected":""}>
      ...
      <div className="stored-backup-actions"><a className="secondary-link" href={`/api/backups/${backup.id}`}>Download</a><button className="secondary-button" type="button" disabled={pending} onClick={()=>run(backup.id,"preview")}>{pending&&selected===backup.id?"Checking…":"Restore preview"}</button></div>
      ...
      {selected===backup.id&&preview&&summary?<div className="stored-restore-preview"><span>SHA-256 verified · non-destructive recovery</span><b>{summary.missing} missing records will be added</b><small>{summary.present} existing records will remain unchanged · {summary.protectedEvidence} protected evidence records validated</small>
        ...
        <details><summary>Review recovery groups</summary><div className="qualification-list">{summary.groups.map(group=><div key={group.id}><span><strong>{group.label}</strong><small>{group.description}</small></span><span><strong>{group.missing} to restore</strong><small>{group.present} already present{group.protectedEvidence?" · protected evidence":""}</small></span></div>)}</div></details>
        {summary.missing===0?<small>All recoverable records from this backup are already present.</small>:null}
        <label>Type RESTORE<input value={confirm} onChange={event=>setConfirm(event.target.value)} autoComplete="off"/></label><button type="button" className="primary-button" disabled={pending||confirm.trim().toUpperCase()!=="RESTORE"||summary.missing===0} onClick={()=>run(backup.id,"restore")}>{pending?"Restoring…":summary.missing?"Restore missing data":"Nothing to restore"}</button></div>:null}
