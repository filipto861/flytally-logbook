...
export function FlightExpensesEditor({initial=[],aircraftCost=0}:{initial?:FlightExpenseRecord[];aircraftCost?:number}){
  ...
  return <section className="expense-editor" aria-label="Additional flight expenses">
    ...
    {rows.length?<div className="expense-rows">{rows.map((row,index)=><div className="expense-row" key={row.key}>
      ...
      <label>Amount <span className="field-hint" aria-hidden="true">Required</span><input name="expenseAmount" type="number" inputMode="decimal" min="0.01" max="10000000" step="0.01" required value={row.amount} onChange={event=>update(row.key,{amount:event.target.value})}/></label>
      <label>Currency <span className="field-hint" aria-hidden="true">Required</span><input name="expenseCurrency" list={currencyListId} maxLength={3} required value={row.currency} onChange={event=>update(row.key,{currency:event.target.value.toUpperCase().replace(/[^A-Z]/g,"").slice(0,3)})}/></label>
