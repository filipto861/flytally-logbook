...
export function FlightWorkflowProgress({state,activeTab,onSelectTab}:{state:FlightWorkflowState;activeTab:"overview"|"gps"|"logbook";onSelectTab:(tab:"overview"|"gps"|"logbook")=>void}){
  ...
  const stage=(label:string,status:"done"|"current"|"next"|"blocked",detail:string)=><div className={`flight-workflow-step ${status}`}><span aria-hidden="true">{status==="done"?"✓":status==="blocked"?"!":"•"}</span><div><strong>{label}</strong><small>{detail}</small></div></div>;
  ...
  return <section className="flight-workflow" aria-label="Flight record workflow">
    ...
    <div className="flight-workflow-steps">
      ...
      {stage("Review",reviewStatus,state.certified?"Final data reviewed":activeTab==="logbook"?"You are reviewing Logbook data":"Check final Logbook data")}
      {stage("Certify",certifyStatus,state.certified?`Protected as R${state.recordRevision}`:state.locked?"Unlock first":state.blockers?"Resolve blockers first":"Protect the final record")}
      {stage("Share",shareStatus,state.certified?"Available now":"Available after certification")}
    </div>
  </section>;
}
