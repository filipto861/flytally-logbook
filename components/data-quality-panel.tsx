...
export function DataQualityPanel({issues,airportStats,repairAction}:{issues:Row[];airportStats:Row;repairAction:()=>Promise<void>}){
  ...
  return <details className="panel quality-panel" open><summary><span>Detailed data check</span><small>{issues.length?`${problems} problems · ${warnings} warnings`:"No issues found"}</small></summary>
    <div className="quality-meta"><span>Airport catalogue <b>{Number(airportStats.total||0).toLocaleString("en-GB")}</b></span><span>Custom airports <b>{Number(airportStats.own||0).toLocaleString("en-GB")}</b></span><form action={repairAction}><button className="secondary-link">Fill missing fields and rates</button></form></div>
