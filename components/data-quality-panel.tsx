...
export function DataQualityPanel({issues,airportStats,repairAction}:{issues:Row[];airportStats:Row;repairAction:(form:FormData)=>Promise<void>}){
  ...
  return <details className="panel quality-panel" open><summary><span>Detailed data check</span><small>{issues.length?`${problems} problems · ${warnings} warnings`:"No issues found"}</small></summary>
    ...
    <p className="muted">Creates a safety backup first. Editable flights are updated directly. Certified flights keep their previous certified revision, receive an automatic correction revision with the aircraft-profile metadata change, and are re-certified automatically when integrity and FCL.050 checks pass. Any record that cannot be safely re-certified is left for manual review.</p>
