...
const choices=[
  ["total","Celkem"],["ull","ULL"],["easa","EASA"],["picUll","PIC ULL"],["picEasa","PIC EASA"],["landings","Přistání"],
] as const;
...
export function MonthlyChart({data}:{data:MonthlyPoint[]}){
  ...
  return <section className="panel chart-panel">
    <div className="section-heading"><div><p className="eyebrow">VÝVOJ</p><h2>Měsíční přehled</h2></div><div className="segment-control">{choices.map(([key,label])=><button className={metric===key?"active":""} key={key} onClick={()=>setMetric(key)}>{label}</button>)}</div></div>
