    ...
    {visible.length?<>
      ...
      <div className="monthly-chart" aria-label={`Monthly trend: ${choices.find(([key])=>key===metric)?.[1]}`}>
        <svg viewBox={`0 0 ${width} ${height}`} role="img" onMouseLeave={()=>setActive(null)}>
          <defs><linearGradient id="monthlyBarGradient" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stopColor="var(--chart-primary)"/><stop offset="1" stopColor="var(--chart-primary-strong)"/></linearGradient><linearGradient id="monthlyBarActive" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stopColor="var(--chart-secondary)"/><stop offset="1" stopColor="var(--chart-secondary-strong)"/></linearGradient></defs>
