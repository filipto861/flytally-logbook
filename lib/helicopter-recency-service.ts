...
const helicopterIr=(row:Record<string,unknown>)=>hasConfirmedQualificationStructure(row)?confirmedQualificationMatches(row,"INSTRUMENT","HELICOPTER","PILOT"):/^IR\s*\(H\)$/i.test(t(row.qualification_type));
const helicopterNight=(row:Record<string,unknown>)=>hasConfirmedQualificationStructure(row)?confirmedQualificationMatches(row,"OPERATIONAL","HELICOPTER","PILOT")&&upper(qualificationLogicScope(row)).includes("NIGHT"):upper(row.qualification_type).includes("NIGHT")&&upper(row.qualification_type).includes("H");
...
export type HelicopterRecencyState={
  ...
  sourceFlights:HelicopterFlight[];
};

export async function getHelicopterRecencyStateForUser(userId:number):Promise<HelicopterRecencyState|null>{
  await ensureDatabaseOptimizations();await Promise.all([ensureV162Schema(),ensureV163Schema(),ensureV165Schema()]);const today=todayIso();
  ...
  const hasHelicopterLicence=activeLicences.length>0,hasLaplH=activeLicences.some(row=>upper(row.licence_type)==="LAPL(H)"),hasIrH=qualifications.some(row=>helicopterIr(row)&&currentCredential(row,today)),hasNightPrivilege=hasIrH||qualifications.some(row=>helicopterNight(row)&&currentCredential(row,today));
  const flights:HelicopterFlight[]=flightRows.map(row=>({id:Number(row.id)||0,registration:t(row.registration),departure:t(row.departure),arrival:t(row.arrival),date:t(row.date).slice(0,10),helicopterType:t(row.helicopter_type),regulatoryCategory:t(row.regulatory_category),role:t(row.role),minutes:flightMinutes(row.off_block,row.on_block),movementEvidenceRecorded:Boolean(row.movement_evidence_recorded),takeoffsDay:Number(row.takeoffs_day)||0,takeoffsNight:Number(row.takeoffs_night)||0,approachesDay:Number(row.approaches_day)||0,approachesNight:Number(row.approaches_night)||0,landingsDay:Number(row.landings_day)||0,landingsNight:Number(row.landings_night)||0,purposeCode:t(row.purpose_code),instructorSigned:Boolean(row.instructor_signed)}));
  ...
  return{today,hasLaplH,hasHelicopterLicence,hasNightPrivilege,hasIrH,helicopterTypes,evaluations,passengerEvaluations,evidence,sourceFlights:flights};
}
