...
export function flightCertificationPayload(row:Record<string,unknown>,userId:number,version=Number(row.certification_version||4)){
  const base={
  ...
  const v3={...v2,version:3,purposeCode:text(row.purpose_code)};
  if(version===3)return v3;
  if(version===4)return{...v3,version:4,movementEvidence:{recorded:Boolean(row.movement_evidence_recorded),takeoffsDay:number(row.takeoffs_day),takeoffsNight:number(row.takeoffs_night),approachesDay:number(row.approaches_day),approachesNight:number(row.approaches_night)}};
  return null;
}

export function flightCertificationHash(row:Record<string,unknown>,userId:number,version=Number(row.certification_version||4)){
