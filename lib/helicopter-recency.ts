...
export function evaluateLaplH(flights:HelicopterFlight[],today:string,helicopterType:string,evidence:HelicopterProficiencyEvidence[]=[]):RecencyEvaluation{
  ...
  const requirements=[requirement("flight-time","Flight time on type",eligible.reduce((sum,f)=>sum+Math.max(0,f.minutes),0)/60,6,"hours"),requirement("movements","Take-offs / approaches / landings",movementCount,6,"count"),requirement("refresher","Signed refresher training ≥ 1 h",refresher.length,1,"count")],current=requirements.every(item=>item.met),movementRequirement=requirements.find(item=>item.id==="movements"),otherRequirementsMet=requirements.filter(item=>item.id!=="movements").every(item=>item.met),limited=!current&&!movementRequirement?.met&&otherRequirementsMet&&incompleteMovementFlights.length>0;
  ...
  return{id:`lapl-h-${typeKey(helicopterType).toLowerCase().replace(/[^a-z0-9]+/g,"-")}`,code:"FCL.140.H",title:`LAPL(H) · ${helicopterType}`,status:current?"current":limited?"attention":"not-current",badge:current?"CURRENT":limited?"LIMITED DATA":"NOT CURRENT",summary:current?"Current on the 12-month experience route":limited?limitedMovementSummary(incompleteMovementFlights.length):`Remaining — ${missing(requirements)}`,windowLabel:"Specific type · last 12 months",requirements,note:limited?"Flight time and signed refresher evidence are recorded, but FlyTally cannot determine the movement element from the available structured counters. Review the contributing certified flights before treating this indication as not current.":"Only certified helicopter flights on this specific stored aircraft type are counted. Dual and supervised-solo experience counts only with instructor-signed evidence. Movement credit requires explicit take-off, approach and landing counters.",meta:{helicopterType,evidenceFlightIds:sourceIds(eligible),trainingFlightIds:sourceIds(refresher),incompleteMovementFlights:incompleteMovementFlights.length,incompleteMovementFlightIds:sourceIds(incompleteMovementFlights)}};
}
...
export function evaluateHelicopterPassengerCurrency(flights:HelicopterFlight[],today:string,helicopterType:string,night=false,hasIrH=false):RecencyEvaluation{
  const start=rollingDaysStart(today,90),window=flights.filter(f=>within(f,start,today)&&upper(f.regulatoryCategory)==="HELICOPTER"&&matchingType(f,helicopterType)&&pfRole(f)),movements=movementTriples(window),nightMovements=nightMovementTriples(window),requirements=[requirement("movements","PF take-offs / approaches / landings",movements,3,"count"),...(night&&!hasIrH?[requirement("night-movement","Night PF take-off / approach / landing",nightMovements,1,"count")]:[])],current=requirements.every(item=>item.met),contributors=window.filter(movementContributor),incompleteMovementFlights=window.filter(f=>!f.movementEvidenceRecorded),limited=!current&&incompleteMovementFlights.length>0;
  return{id:`helicopter-passenger-${night?"night-":""}${typeKey(helicopterType).toLowerCase().replace(/[^a-z0-9]+/g,"-")}`,code:"FCL.060",title:`Helicopter passenger currency · ${helicopterType}${night?" · night":""}`,status:current?"current":limited?"attention":"not-current",badge:current?"CURRENT":limited?"LIMITED DATA":"NOT CURRENT",summary:current?"Passenger recent-experience requirement satisfied":limited?limitedMovementSummary(incompleteMovementFlights.length):`Remaining — ${missing(requirements)}`,windowLabel:"Preceding 90 days",requirements,note:limited?"Relevant certified flights exist without structured PF movement counters, so FlyTally cannot determine this recent-experience indication from saved evidence alone.":night&&hasIrH?"A current IR(H) satisfies the separate night-PIC condition; the three PF take-off/approach/landing movements remain required.":night?"Night PIC passenger carriage requires at least one qualifying night take-off, approach and landing unless a current IR(H) is held.":"Counts only explicit PF take-off, approach and landing evidence on this stored helicopter type.",meta:{helicopterType,night,hasIrH,evidenceFlightIds:sourceIds(limited?window:contributors),incompleteMovementFlights:incompleteMovementFlights.length,incompleteMovementFlightIds:sourceIds(incompleteMovementFlights)}};
}
