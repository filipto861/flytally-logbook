...
export type HelicopterProficiencyEvidence={
  id:number;
  helicopterType:string;
  date:string;
  signer:string;
...
const movementTriples=(flights:HelicopterFlight[])=>{
  const explicit=flights.filter(flight=>flight.movementEvidenceRecorded);
  const takeoffs=explicit.reduce((sum,flight)=>sum+Math.max(0,flight.takeoffsDay)+Math.max(0,flight.takeoffsNight),0);
  const approaches=explicit.reduce((sum,flight)=>sum+Math.max(0,flight.approachesDay)+Math.max(0,flight.approachesNight),0);
  const landings=explicit.reduce((sum,flight)=>sum+Math.max(0,flight.landingsDay)+Math.max(0,flight.landingsNight),0);
  return Math.min(takeoffs,approaches,landings);
};
const nightMovementTriples=(flights:HelicopterFlight[])=>{
  const explicit=flights.filter(flight=>flight.movementEvidenceRecorded);
  const takeoffs=explicit.reduce((sum,flight)=>sum+Math.max(0,flight.takeoffsNight),0);
  const approaches=explicit.reduce((sum,flight)=>sum+Math.max(0,flight.approachesNight),0);
  const landings=explicit.reduce((sum,flight)=>sum+Math.max(0,flight.landingsNight),0);
  return Math.min(takeoffs,approaches,landings);
};
