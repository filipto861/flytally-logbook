...
export function validateBackupCertificationHistory(backup:PortableBackup,currentUserId:number):BackupCertificationSummary{
  ...
  const flightRevisionMap=new Map<string,BackupRow[]>();for(const revision of backup.flight_certified_revisions){const key=String(revision.flight_id??""),items=flightRevisionMap.get(key)??[];items.push(revision);flightRevisionMap.set(key,items)}
  const fstdRevisionMap=new Map<string,BackupRow[]>();for(const revision of backup.fstd_certified_revisions){const key=String(revision.fstd_session_id??""),items=fstdRevisionMap.get(key)??[];items.push(revision);fstdRevisionMap.set(key,items)}
