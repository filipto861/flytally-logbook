...
export async function getSplRecencyStateForUser(userId:number):Promise<SplRecencyState|null>{
  await ensureDatabaseOptimizations();await ensureV162Schema();const today=todayIso();
  ...
  const hasSailplanePrivilege=splQualifications.some(row=>privilegeType(row.qualification_type)==="SAILPLANE")||flights.some(row=>upper(row.aircraftClass)!=="TMG"),hasTmgPrivilege=splQualifications.some(row=>privilegeType(row.qualification_type)==="TMG")||flights.some(row=>upper(row.aircraftClass)==="TMG");
  const partFclTmgPrivilege=qualifications.some(row=>{if(privilegeType(row.qualification_type)!=="TMG")return false;const parent=licenceById.get(Number(row.licence_id)),licence=upper(parent?.licence_type);if(!["LAPL(A)","PPL(A)","CPL(A)","ATPL(A)"].includes(licence))return false;const state=credentialValidity({mode:row.validity_mode,validUntil:row.valid_until,recencyUntil:row.recency_until},today);return state.status!=="expired"&&state.status!=="incomplete"});
  const methodSet=new Set<LaunchMethod>();for(const row of splQualifications){const method=launchMethodFromQualification(row.qualification_type);if(method)methodSet.add(method)}for(const flight of flights){const method=upper(flight.launchMethod) as LaunchMethod;if(["WINCH","AEROTOW","SELF_LAUNCH","CAR","BUNGEE"].includes(method))methodSet.add(method)}
