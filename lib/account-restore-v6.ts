...
const splEvidenceKey=(row:BackupRow)=>`${upper(row.aircraft_context)}|${text(row.evidence_date).slice(0,10)}|${upper(row.signer_name)}|${upper(row.reference)}`;
...
export async function prepareExactAccountRestore(userId:number,backup:PortableBackup,digest:string):Promise<ExactRestorePlan>{
  ...
  const [flights,aircraft,rates,airports,expiries,tracks,points,fstd,flightRevisions,fstdRevisions,audit,deleted,expenses,splEvidence]=await Promise.all([
    ...
    sql`SELECT id,aircraft_context,evidence_date::text evidence_date,signer_name,reference FROM spl_recency_evidence WHERE user_id=${userId}`,
