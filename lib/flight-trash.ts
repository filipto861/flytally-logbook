...
export async function moveFlightToTrash(userId:number,flightId:number):Promise<boolean>{
  await ensureDatabaseOptimizations();await Promise.all([ensureV159Schema(),ensureV162Schema(),ensureV164Schema()]);const token=randomUUID();
...
export async function restoreDeletedFlightRecord(userId:number,trashId:number):Promise<{flightId?:number;error?:string}>{
  await ensureDatabaseOptimizations();await Promise.all([ensureV159Schema(),ensureV162Schema(),ensureV164Schema()]);
  const rows=await sql`WITH candidate AS MATERIALIZED (
      ...
      INSERT INTO flights(user_id,date,evidence,registration,aircraft_type,aircraft_class,regulatory_category,balloon_class,balloon_group,launch_method,launches,departure,arrival,off_block,takeoff,landing,on_block,starts,commander,instructor,role,task,purpose_code,price_per_hour,billing_basis,note,locked_at,locked_by_user_id,operation_type,engine_type,landings_day,landings_night,movement_evidence_recorded,takeoffs_day,takeoffs_night,approaches_day,approaches_night,night_minutes,ifr_minutes,pic_minutes,copilot_minutes,dual_minutes,instructor_minutes,verification_name,verification_reference)
      SELECT ${userId},NULLIF(flight_data->>'date','')::date,COALESCE(flight_data->>'evidence',''),COALESCE(flight_data->>'registration',''),COALESCE(flight_data->>'aircraft_type',''),COALESCE(flight_data->>'aircraft_class',''),COALESCE(flight_data->>'regulatory_category',''),COALESCE(flight_data->>'balloon_class',''),COALESCE(flight_data->>'balloon_group',''),COALESCE(flight_data->>'launch_method',''),COALESCE(NULLIF(flight_data->>'launches','')::int,0),COALESCE(flight_data->>'departure',''),COALESCE(flight_data->>'arrival',''),COALESCE(flight_data->>'off_block',''),COALESCE(flight_data->>'takeoff',''),COALESCE(flight_data->>'landing',''),COALESCE(flight_data->>'on_block',''),COALESCE(NULLIF(flight_data->>'starts','')::int,0),COALESCE(flight_data->>'commander',''),COALESCE(flight_data->>'instructor',''),COALESCE(flight_data->>'role',''),COALESCE(flight_data->>'task',''),COALESCE(flight_data->>'purpose_code',''),NULLIF(flight_data->>'price_per_hour','')::numeric,COALESCE(flight_data->>'billing_basis','BLOCK'),COALESCE(flight_data->>'note',''),NULL,NULL,COALESCE(flight_data->>'operation_type','SP'),COALESCE(flight_data->>'engine_type','SE'),COALESCE(NULLIF(flight_data->>'landings_day','')::int,COALESCE(NULLIF(flight_data->>'starts','')::int,0)),COALESCE(NULLIF(flight_data->>'landings_night','')::int,0),COALESCE(NULLIF(flight_data->>'movement_evidence_recorded','')::boolean,FALSE),COALESCE(NULLIF(flight_data->>'takeoffs_day','')::int,0),COALESCE(NULLIF(flight_data->>'takeoffs_night','')::int,0),COALESCE(NULLIF(flight_data->>'approaches_day','')::int,0),COALESCE(NULLIF(flight_data->>'approaches_night','')::int,0),COALESCE(NULLIF(flight_data->>'night_minutes','')::int,0),COALESCE(NULLIF(flight_data->>'ifr_minutes','')::int,0),COALESCE(NULLIF(flight_data->>'pic_minutes','')::int,0),COALESCE(NULLIF(flight_data->>'copilot_minutes','')::int,0),COALESCE(NULLIF(flight_data->>'dual_minutes','')::int,0),COALESCE(NULLIF(flight_data->>'instructor_minutes','')::int,0),COALESCE(flight_data->>'verification_name',''),COALESCE(flight_data->>'verification_reference','')
