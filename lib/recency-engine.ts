...
export function evaluatePassengerCurrencyMode(flights:RecencyFlight[],aircraftClass:"SEP"|"TMG",hasIr:boolean,today:string,mode:"day"|"night"):RecencyEvaluation{
  ...
  if(mode==="night"&&hasIr)return{id:`fcl060-${aircraftClass.toLowerCase()}-night`,code:"FCL.060",title:`${aircraftClass} night passenger currency`,status:"current",summary:"Current IR exemption applies",windowLabel:"Preceding 90 days",requirements:[requirement("night-passengers","IR exemption",1,1,"count")],note:"FlyTally treats the saved current IR as the night take-off/landing currency exemption. Authority records remain controlling.",meta:{landings,nightLandings:night,irExemption:true}};
  const current=mode==="day"?landings:night,target=mode==="day"?3:1,label=mode==="day"?"Landings":"Night landings",req=requirement(mode==="day"?"day-passengers":"night-passengers",label,current,target,"count"),met=req.met,forecastDate=met?contributionForecast(landingContributions(window,mode==="night"),target,date=>addDays(date,90)):undefined;
  return{id:`fcl060-${aircraftClass.toLowerCase()}-${mode}`,code:"FCL.060",title:mode==="day"?`${aircraftClass} passenger currency`:`${aircraftClass} night passenger currency`,status:met?"current":"not-current",summary:met?(mode==="day"?"Passenger currency current":"Night passenger currency current"):`${mode==="day"?"Passenger":"Night passenger"} currency not current · ${Math.max(0,Math.ceil(target-current))} landing${Math.max(0,Math.ceil(target-current))===1?"":"s"} remaining`,windowLabel:"Preceding 90 days",requirements:[req],forecastDate,note:"Planning indicator based on certified flights and recorded landings. FlyTally does not currently record take-offs/approaches or FFS pilot-flying events separately.",meta:{landings,nightLandings:night,irExemption:false}};
}
export function evaluatePassengerCurrency(flights:RecencyFlight[],aircraftClass:"SEP"|"TMG",hasIr:boolean,today:string):RecencyEvaluation{
  const day=evaluatePassengerCurrencyMode(flights,aircraftClass,hasIr,today,"day"),night=evaluatePassengerCurrencyMode(flights,aircraftClass,hasIr,today,"night"),status:RecencyStatus=day.status==="current"?(night.status==="current"?"current":"attention"):"not-current";
  return{id:`fcl060-${aircraftClass.toLowerCase()}`,code:"FCL.060",title:`${aircraftClass} passenger currency`,status,summary:day.status==="current"?(night.status==="current"?"Day and night passenger currency current":"Day current · night PIC not current"):day.summary,windowLabel:"Preceding 90 days",requirements:[...day.requirements,...night.requirements],forecastDate:day.forecastDate,note:day.note,meta:day.meta};
}
