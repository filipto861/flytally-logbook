...
import { deliverPushNotification } from "@/lib/push-notifications";
...
export async function notifyUser(userId:number,input:NotificationInput){
  ...
  const rows=await sql`INSERT INTO user_notifications(user_id,kind,title,body,href,dedupe_key) VALUES(${userId},${input.kind.slice(0,60)},${input.title.slice(0,160)},${String(input.body??"").slice(0,500)},${String(input.href??"").slice(0,500)},${input.dedupeKey.slice(0,200)}) ON CONFLICT(user_id,dedupe_key) DO UPDATE SET title=EXCLUDED.title,body=EXCLUDED.body,href=EXCLUDED.href,created_at=NOW(),read_at=NULL RETURNING id` as Array<{id:number|string}>;
  if(rows[0])await deliverPushNotification(userId,input.kind);
}
export async function notifyUserOnce(userId:number,input:NotificationInput){
  if(!Number.isSafeInteger(userId)||userId<=0)return;
  const rows=await sql`INSERT INTO user_notifications(user_id,kind,title,body,href,dedupe_key) VALUES(${userId},${input.kind.slice(0,60)},${input.title.slice(0,160)},${String(input.body??"").slice(0,500)},${String(input.href??"").slice(0,500)},${input.dedupeKey.slice(0,200)}) ON CONFLICT(user_id,dedupe_key) DO NOTHING RETURNING id` as Array<{id:number|string}>;
  if(rows[0])await deliverPushNotification(userId,input.kind);
}
