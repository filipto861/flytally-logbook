...
export type BalloonRecencyState={today:string;hasBpl:boolean;heldClasses:BalloonClass[];classLabels:Record<BalloonClass,string>;anchorClass:BalloonClass|null;current:boolean;anchor:ReturnType<typeof evaluateBplBaseClass>|null;additional:ReturnType<typeof evaluateBplAdditionalClass>[];candidates:Array<{balloonClass:BalloonClass;evaluation:ReturnType<typeof evaluateBplBaseClass>}>;evidence:BalloonProficiencyEvidence[];sourceFlights:BalloonFlight[];hasTetheredRating:boolean;tethered:ReturnType<typeof evaluateBplTetheredRating>|null};
...
export async function getBalloonRecencyStateForUser(userId:number):Promise<BalloonRecencyState|null>{
  ...
  const[licences,qualifications,flightRows,evidenceRows]=await Promise.all([
    ...
    sql`SELECT f.id,f.date::text date,f.registration,f.departure,f.arrival,f.regulatory_category,f.balloon_class,f.balloon_group,f.balloon_operation,f.role,f.takeoff,f.landing,f.takeoffs_day,f.takeoffs_night,f.landings_day,f.landings_night,f.purpose_code,
  ...
  const flights:BalloonFlight[]=flightRows.map(row=>({id:Number(row.id)||0,registration:t(row.registration),departure:t(row.departure),arrival:t(row.arrival),date:t(row.date).slice(0,10),regulatoryCategory:t(row.regulatory_category),balloonClass:t(row.balloon_class),balloonGroup:t(row.balloon_group),balloonOperation:t(row.balloon_operation),role:t(row.role),airMinutes:flightMinutes(row.takeoff,row.landing),takeoffs:(Number(row.takeoffs_day)||0)+(Number(row.takeoffs_night)||0),landings:(Number(row.landings_day)||0)+(Number(row.landings_night)||0),purposeCode:t(row.purpose_code),instructorSigned:Boolean(row.instructor_signed)}));
  ...
  return{today,hasBpl,heldClasses,classLabels:{HOT_AIR_BALLOON:classLabel("HOT_AIR_BALLOON"),GAS_BALLOON:classLabel("GAS_BALLOON"),HOT_AIR_AIRSHIP:classLabel("HOT_AIR_AIRSHIP"),MIXED_BALLOON:classLabel("MIXED_BALLOON")},anchorClass:result.anchorClass,current:result.current,anchor:result.anchor,additional:result.additional,candidates,evidence,sourceFlights:flights,hasTetheredRating,tethered:hasTetheredRating?evaluateBplTetheredRating(flights,today):null};
}
