...
const qualificationClass=(value:unknown):BalloonClass|null=>{const normalized=upper(value).replace(/[_-]+/g," ").replace(/\s+/g," ");if(normalized==="HOT AIR BALLOON")return "HOT_AIR_BALLOON";if(normalized==="GAS BALLOON")return "GAS_BALLOON";if(normalized==="HOT AIR AIRSHIP")return "HOT_AIR_AIRSHIP";if(normalized==="MIXED BALLOON")return "MIXED_BALLOON";return null;};
const tetheredQualification=(value:unknown)=>{const normalized=upper(value).replace(/[_-]+/g," ").replace(/\s+/g," ");return normalized.includes("TETHERED")&&normalized.includes("BALLOON")||normalized==="TETHERED"};
...
export async function getBalloonRecencyStateForUser(userId:number):Promise<BalloonRecencyState|null>{
  ...
  const heldClasses=[...new Set(qualifications.map(row=>qualificationClass(row.qualification_type)).filter((value):value is BalloonClass=>Boolean(value)))],hasTetheredRating=qualifications.some(row=>tetheredQualification(row.qualification_type));
