...
export type AircraftOption = { registration: string; aircraft_type: string; aircraft_make:string; aircraft_model:string; aircraft_variant:string; aircraft_class: string; regulatory_category:string; evidence: string; default_role: string; billing_basis: string; price_per_hour: number };
...
export async function getAircraftOptions(userId: number) {
  ...
  return await sql`
    SELECT a.registration, COALESCE(a.aircraft_type, '') AS aircraft_type,
           ...
           COALESCE(aircraft_class, '') AS aircraft_class, COALESCE(regulatory_category,'') AS regulatory_category, COALESCE(evidence, '') AS evidence,
