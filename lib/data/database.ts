...
export async function getDatabaseData(userId:number){
  const [aircraft,rates,airports,quality,issues,airportStats,routeCodes]=await Promise.all([
    ...
    sql`WITH base AS (SELECT f.*,a.registration profile_registration,a.evidence profile_evidence,a.aircraft_class profile_class,a.aircraft_type profile_type FROM flights f LEFT JOIN aircraft a ON a.user_id=f.user_id AND UPPER(TRIM(a.registration))=UPPER(TRIM(f.registration)) WHERE f.user_id=${userId}), duplicates AS (SELECT id,ROW_NUMBER() OVER(PARTITION BY date,UPPER(TRIM(registration)),UPPER(TRIM(departure)),UPPER(TRIM(arrival)),off_block,takeoff,landing,on_block ORDER BY id) duplicate_rank FROM base), findings AS (
      ...
      UNION ALL SELECT id,'problem','invalid_easa_category','Invalid EASA category','Operation must be SP/MP and engine category SE/ME.' FROM base WHERE UPPER(COALESCE(evidence,''))='EASA' AND (UPPER(COALESCE(operation_type,'')) NOT IN ('SP','MP') OR UPPER(COALESCE(engine_type,'')) NOT IN ('SE','ME'))
      UNION ALL SELECT id,'problem','missing_easa_verification','PICUS/SPIC verification missing','Enter the supervising pilot in the EASA section.' FROM base WHERE UPPER(COALESCE(evidence,''))='EASA' AND UPPER(COALESCE(role,'')) IN ('PICUS','SPIC') AND NULLIF(TRIM(COALESCE(verification_name,'')),'') IS NULL
      UNION ALL SELECT id,'problem','easa_time_exceeds_total','EASA condition time exceeds total time','Night or IFR time cannot exceed BLOCK time.' FROM base WHERE UPPER(COALESCE(evidence,''))='EASA' AND off_block ~ '^([01][0-9]|2[0-3]):[0-5][0-9]$' AND on_block ~ '^([01][0-9]|2[0-3]):[0-5][0-9]$' AND (COALESCE(night_minutes,0)>MOD((split_part(on_block,':',1)::int*60+split_part(on_block,':',2)::int)-(split_part(off_block,':',1)::int*60+split_part(off_block,':',2)::int)+1440,1440) OR COALESCE(ifr_minutes,0)>MOD((split_part(on_block,':',1)::int*60+split_part(on_block,':',2)::int)-(split_part(off_block,':',1)::int*60+split_part(off_block,':',2)::int)+1440,1440))
