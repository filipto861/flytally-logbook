...
export async function getDashboardData(userId:number,requested:string):Promise<DashboardData>{
  ...
  const rows=await measureServerTask("dashboard-data",()=>sql`WITH track AS MATERIALIZED(
      ...
      SELECT
        COUNT(*) FILTER(WHERE dashboard_total)::int total_flights,COALESCE(SUM(activity_minutes) FILTER(WHERE dashboard_total),0)::int total_minutes,COALESCE(SUM(landings) FILTER(WHERE dashboard_total AND NOT auxiliary),0)::int total_landings,
        COUNT(*) FILTER(WHERE evidence='ULL' AND NOT auxiliary)::int ull_flights,COALESCE(SUM(logged_minutes) FILTER(WHERE evidence='ULL' AND NOT auxiliary),0)::int ull_minutes,COALESCE(SUM(landings) FILTER(WHERE evidence='ULL' AND NOT auxiliary),0)::int ull_landings,
        COUNT(*) FILTER(WHERE evidence='EASA' AND NOT auxiliary)::int easa_flights,COALESCE(SUM(logged_minutes) FILTER(WHERE evidence='EASA' AND NOT auxiliary),0)::int easa_minutes,COALESCE(SUM(landings) FILTER(WHERE evidence='EASA' AND NOT auxiliary),0)::int easa_landings,
    ...
    SELECT COALESCE((SELECT display_name FROM users WHERE id=${userId}),'Pilot') display_name,summary.*,
      ...
      COALESCE((SELECT jsonb_agg(to_jsonb(m) ORDER BY m.month_key) FROM(SELECT LEFT(date_key,7) AS month_key,COALESCE(SUM(activity_minutes) FILTER(WHERE dashboard_total),0)::int total,COALESCE(SUM(logged_minutes) FILTER(WHERE NOT auxiliary AND evidence='ULL'),0)::int ull,COALESCE(SUM(logged_minutes) FILTER(WHERE NOT auxiliary AND evidence='EASA'),0)::int easa,COALESCE(SUM(pic_minutes) FILTER(WHERE NOT auxiliary AND evidence='ULL' AND pic_minutes>0),0)::int pic_ull,COALESCE(SUM(pic_minutes) FILTER(WHERE NOT auxiliary AND evidence='EASA' AND pic_minutes>0),0)::int pic_easa,COALESCE(SUM(landings) FILTER(WHERE NOT auxiliary),0)::int landings FROM base WHERE dashboard_total AND date_key IS NOT NULL GROUP BY LEFT(date_key,7))m),'[]'::jsonb) monthly,
      COALESCE((SELECT jsonb_agg(to_jsonb(a) ORDER BY a.minutes DESC,a.registration) FROM(SELECT registration,COUNT(*) FILTER(WHERE NOT auxiliary)::int flights,COALESCE(SUM(logged_minutes) FILTER(WHERE NOT auxiliary),0)::int minutes,COALESCE(SUM(cost),0)::double precision cost,COALESCE(MAX(date_key),'') last_date FROM base WHERE registration<>'' GROUP BY registration)a),'[]'::jsonb) top_aircraft,
      COALESCE((SELECT jsonb_agg(to_jsonb(r) ORDER BY r.flights DESC,r.minutes DESC,r.last_date DESC,r.departure,r.arrival) FROM(SELECT departure||'→'||arrival route,departure,arrival,COUNT(*)::int flights,COALESCE(SUM(activity_minutes),0)::int minutes,COALESCE(MIN(date_key),'') first_date,COALESCE(MAX(date_key),'') last_date FROM base WHERE dashboard_total AND departure<>'' AND arrival<>'' GROUP BY departure,arrival ORDER BY COUNT(*) DESC,SUM(activity_minutes) DESC,MAX(date_key) DESC NULLS LAST LIMIT 50)r),'[]'::jsonb) top_routes,
      ...
      COALESCE((SELECT jsonb_agg(to_jsonb(y) ORDER BY y.year_key DESC) FROM(SELECT LEFT(date_key,4)::int AS year_key,COUNT(*)::int flights,COALESCE(SUM(activity_minutes),0)::int minutes,COALESCE(SUM(landings) FILTER(WHERE NOT auxiliary),0)::int landings FROM base WHERE dashboard_total AND date_key IS NOT NULL GROUP BY LEFT(date_key,4))y),'[]'::jsonb) yearly
