...
export async function getDashboardData(userId:number,requested:string):Promise<DashboardData>{
  ...
  const settled=await Promise.allSettled([
    ...
    sql`SELECT id,date,COALESCE(registration,'') registration,COALESCE(departure,'') departure,COALESCE(arrival,'') arrival FROM flights f WHERE user_id=${userId} AND (${start}::date IS NULL OR f.date::date>=${start}::date) AND (${end}::date IS NULL OR f.date::date<=${end}::date) ORDER BY date DESC,off_block DESC NULLS LAST,id DESC LIMIT 8`
  ]);
  ...
  if(!result[0].length)return fallbackDashboard(userId,label,start,end);
