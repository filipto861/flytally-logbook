...
export async function getOverviewTracks(userId: number, limit = 100,filters:MapFilters={}) {
  ...
  const rows = await sql`
    ...
    WHERE t.user_id=${userId} AND t.overview_coordinates_json IS NOT NULL
      ...
      AND (${year}::text IS NULL OR LEFT(f.date::text,4)=${year}::text)
...
export async function getRouteOverview(userId:number,filters:MapFilters={}){
  ...
  const [routeRows,airportRows,customAirportRows]=await Promise.all([
    sql`WITH b AS (SELECT UPPER(TRIM(f.departure)) departure,UPPER(TRIM(f.arrival)) arrival,CASE WHEN f.off_block ~ '^([01][0-9]|2[0-3]):[0-5][0-9]$' AND f.on_block ~ '^([01][0-9]|2[0-3]):[0-5][0-9]$' THEN MOD((split_part(f.on_block,':',1)::int*60+split_part(f.on_block,':',2)::int)-(split_part(f.off_block,':',1)::int*60+split_part(f.off_block,':',2)::int)+1440,1440) ELSE 0 END minutes FROM flights f WHERE f.user_id=${userId} AND NULLIF(TRIM(f.departure),'') IS NOT NULL AND NULLIF(TRIM(f.arrival),'') IS NOT NULL AND (${registration}::text IS NULL OR UPPER(f.registration)=UPPER(${registration}::text)) AND (${evidence}::text IS NULL OR UPPER(f.evidence)=UPPER(${evidence}::text)) AND (${airport}::text IS NULL OR UPPER(f.departure)=UPPER(${airport}::text) OR UPPER(f.arrival)=UPPER(${airport}::text)) AND (${route}::text IS NULL OR UPPER(CONCAT(f.departure,'→',f.arrival))=UPPER(${route}::text)) AND (${year}::text IS NULL OR LEFT(f.date::text,4)=${year}::text)) SELECT departure,arrival,COUNT(*)::int flights,SUM(minutes)::int minutes FROM b GROUP BY 1,2 ORDER BY flights DESC,departure,arrival`,
    sql`WITH b AS (SELECT UPPER(TRIM(f.departure)) departure,UPPER(TRIM(f.arrival)) arrival FROM flights f WHERE f.user_id=${userId} AND NULLIF(TRIM(f.departure),'') IS NOT NULL AND NULLIF(TRIM(f.arrival),'') IS NOT NULL AND (${registration}::text IS NULL OR UPPER(f.registration)=UPPER(${registration}::text)) AND (${evidence}::text IS NULL OR UPPER(f.evidence)=UPPER(${evidence}::text)) AND (${airport}::text IS NULL OR UPPER(f.departure)=UPPER(${airport}::text) OR UPPER(f.arrival)=UPPER(${airport}::text)) AND (${route}::text IS NULL OR UPPER(CONCAT(f.departure,'→',f.arrival))=UPPER(${route}::text)) AND (${year}::text IS NULL OR LEFT(f.date::text,4)=${year}::text)),movements AS (SELECT departure ident FROM b UNION ALL SELECT arrival FROM b) SELECT ident,COUNT(*)::int flights FROM movements GROUP BY ident ORDER BY flights DESC,ident`,
