...
def render_sidebar_restore_button() -> None:
    ...
    if not st.session_state.get("sidebar_hidden", False):
        return
    cols = st.columns([0.18, 5.82])
    with cols[0]:
        if st.button("›", key="show_manual_sidebar", help="Zobrazit menu"):
