...
3. Before publishing a candidate, run `npm run verify`. This runs the complete unit/regression suite once and then performs the real Next.js production build.
...
Every pull request gets the **Fast application gate**: all unit/regression tests plus `next build`. Because `next build` already performs TypeScript validation, CI does not run a second standalone typecheck.
...
- `npm run verify:release` — complete unit/regression, PostgreSQL and production-build verification when a suitable PostgreSQL test database is configured.
