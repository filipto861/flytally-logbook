...
test("aircraft cards use one uniform media layout whether a photo exists or not",()=>{
  ...
  assert.match(sharingCss,/\.aircraft-card\{[^}]*grid-template-columns:116px minmax\(0,1fr\)/);
  assert.match(sharingCss,/grid-template-areas:"cover header" "summary summary" "action action"/);
  assert.match(sharingCss,/\.aircraft-card-list\{[^}]*align-items:stretch/);
  assert.match(sharingCss,/\.aircraft-card-cover img\{[^}]*object-fit:contain/);
  assert.match(sharingCss,/@media\(max-width:760px\)\{[\s\S]*grid-template-columns:92px minmax\(0,1fr\)/);
  assert.match(sharingCss,/@media\(max-width:760px\)\{[\s\S]*grid-template-areas:"cover header" "summary summary" "action action"/);
});
