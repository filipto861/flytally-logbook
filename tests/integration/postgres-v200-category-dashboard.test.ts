...
function queryBlock(){const source=fs.readFileSync(path.join(root,"lib/data/dashboard.ts"),"utf8"),blocks=[...source.matchAll(/sql`([\s\S]*?)`/g)].map(match=>match[1]).filter(value=>value.includes("WITH track AS MATERIALIZED("));const block=blocks.at(-1);assert.ok(block,"v2.0-F legacy Dashboard SQL block not found");return block.replaceAll("\\\\d","\\d")}
function render(){
