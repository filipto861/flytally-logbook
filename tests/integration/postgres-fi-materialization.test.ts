...
function renderMaterialize(block:string,values:Record<string,unknown>){
  const noteExpression="${instructor?`FI entry linked to ${text(row.source_name)}'s verified training flight`:`Shared flight with ${text(row.source_name)}`}";
  assert.ok(block.includes(noteExpression),"Expected instructor materialisation note interpolation");
...
before(()=>{
  ...
  const setup=`
    ...
    CREATE TABLE flight_participations(
      ...
      participant_role TEXT NOT NULL,source_revision INTEGER NOT NULL,source_hash TEXT NOT NULL,status TEXT NOT NULL DEFAULT 'pending',participant_flight_id BIGINT REFERENCES flights(id) ON DELETE SET NULL,responded_at TIMESTAMPTZ
    );
