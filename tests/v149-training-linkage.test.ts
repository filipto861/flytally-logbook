...
const flight=(overrides:Partial<RecencyFlight>={}):RecencyFlight=>({date:"2026-08-20",evidence:"EASA",aircraftClass:"SEP",role:"PIC",minutes:660,landingsDay:11,landingsNight:0,...overrides});
...
test("v1.49 signed FCL.740.A DUAL flight satisfies the refresher element directly",()=>{
  ...
  const flights=[flight(),flight({role:"DUAL",minutes:60,landingsDay:1,purposeCode:"SEP_TMG_FCL740A_REFRESHER",task:"FCL.740.A refresher training",instructorSigned:true})];
...
test("unsigned refresher purpose does not satisfy FCL.740.A",()=>{
  const flights=[flight(),flight({role:"DUAL",minutes:60,landingsDay:1,purposeCode:"SEP_TMG_FCL740A_REFRESHER",task:"FCL.740.A refresher training",instructorSigned:false})];
...
test("combined LAPL plus FCL.740.A purpose remains detectable from certified task",()=>{
  const flights=[flight(),flight({role:"DUAL",minutes:60,landingsDay:1,purposeCode:"LAPL_FCL140A_REFRESHER",task:"Differences training · FCL.140.A refresher training · FCL.740.A refresher training · Circuits",instructorSigned:true})];
...
test("SEP and TMG experience can be combined when both ratings are held",()=>{
  const flights=[flight({aircraftClass:"TMG"}),flight({aircraftClass:"SEP",role:"DUAL",minutes:60,landingsDay:1,purposeCode:"SEP_TMG_FCL740A_REFRESHER",instructorSigned:true})];
...
test("documented FCL.740.A refresher exemption can replace the one-hour refresher element",()=>{
  ...
  const result=evaluateClassRevalidation({aircraftClass:"SEP",validUntil:"2026-09-30",today:"2026-08-31",flights:[flight({minutes:720,landingsDay:12})],evidence});
