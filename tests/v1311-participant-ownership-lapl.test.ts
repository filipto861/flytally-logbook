...
test("participant-owned copy can be removed without deleting the source record or verification",()=>{
  ...
  assert.match(trash,/UPDATE flight_participations SET participant_flight_id=NULL WHERE participant_user_id=\$\{userId\} AND participant_flight_id=\$\{flightId\}/);
...
test("v1.31.1 release version is current",()=>{assert.equal(JSON.parse(read("package.json")).version,"1.31.1")});
