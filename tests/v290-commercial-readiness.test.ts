...
const clearedEnv = {
  ...
  COMMERCIAL_BRAND_CLAIMS_POLICY_VERSION: "2026-09-18-c5-v1",
  COMMERCIAL_RELEASE_AUDIT_VERSION: "2026-09-18-c6-v1",
...
test("v2.9 commercial launch defaults fail closed to private beta", () => {
  ...
  assert.ok(readiness.blockers.includes("brand-claims-evidence"));
  assert.ok(readiness.blockers.includes("commercial-release-audit"));
});
...
test("v2.9 can record complete external validation while C2/C3/C4/C5/C6 code gates remain incomplete", () => {
  ...
  assert.deepEqual(readiness.blockers, ["commercial-legal-publication","commercial-billing-runtime","signature-regulatory-evidence","brand-claims-evidence","commercial-release-audit"]);
  assert.equal(readiness.effectiveStage, "external-validation");
  assert.equal(readiness.commercialEnabled, false);
});
