...
export async function createFstdSession(form:FormData){
  ...
  const sessionDate=text(form,"sessionDate",10),deviceType=text(form,"deviceType",120),qualificationNumber=text(form,"qualificationNumber",120),instruction=text(form,"instruction",300),remarks=text(form,"remarks",2000),totalMinutes=durationMinutes(form.get("totalTime"));
  const draft={session_date:sessionDate,device_type:deviceType,qualification_number:qualificationNumber,instruction,total_minutes:totalMinutes,remarks};
  if(!validDate(sessionDate)||blockingComplianceIssues(fstdCompliance(draft)).length)return;
