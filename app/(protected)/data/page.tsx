...
export default async function DataPage({searchParams}:{searchParams:Promise<Params>}){
  const{userId}=await requireUser(),params=await searchParams,view=resolveView(params.view),timeZonePromise=getUserTimezone(userId);
  ...
  return <div className="ui-page-stack">
    ...
    <DataHub view={view} timeZone={timeZone} backups={backups} deletedFlights={deletedFlights} createAction={createManualBackup} restoreStoredAction={restoreStoredBackup} restoreFileAction={restorePortableBackup} restoreTrashAction={restoreDeletedFlight}/>
  </div>;
}
