...
export default async function StatisticsPage({searchParams}:{searchParams:Promise<{period?:string;section?:string;category?:string}>}){
  ...
  const data=await getPilotInsightsData(session.userId,period,category),s=data.summary,loggedMinutes=s.minutes,pace=paceComparison(data.current12m.minutes,data.previous12m.minutes),picShare=sharePercent(s.picMinutes,loggedMinutes),nightShare=sharePercent(s.nightMinutes,loggedMinutes),ifrShare=sharePercent(s.ifrMinutes,loggedMinutes),scopeLabel=category?categoryLabels[category]||category:"All categories";
