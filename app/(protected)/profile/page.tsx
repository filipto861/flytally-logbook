    ...
    <section className="panel expiry-panel">
      ...
      <div className="credential-list">{otherExpiries.map(e=>{
        ...
        return <details key={t(e.id)} className={`credential-card ${state.className}${Number(e.active)?'':' inactive'}`}>
          ...
          <div className="credential-editor credential-action-editor"><div><p className="eyebrow">RECORD ACTIONS</p><p className="muted">Archive this item when it is no longer current, or delete it permanently.</p></div><div className="credential-record-actions"><form action={toggleExpiry}><input type="hidden" name="id" value={t(e.id)}/><button className="secondary-link">{Number(e.active)?'Archive':'Activate'}</button></form><form action={deleteExpiry}><input type="hidden" name="id" value={t(e.id)}/><button className="icon-danger">Delete</button></form></div></div>
        </details>;
