      ...
      <main className="u33-settings-workspace">
        ...
        <form action={saveAccountSettings} className="account-settings-form u33-general-form">
          <div className="profile-grid">
            <section className="panel"><p className="eyebrow">PERSONAL</p><h2>Pilot details</h2><div className="stack-form"><label>Name<input name="display_name" defaultValue={t(user.display_name)} required/></label><label>Account email<input type="email" value={t(user.email)} readOnly/><small>Your sign-in identity. Email changes require verification.</small></label></div></section>
          ...
          <div className="settings-save"><span>Personal details and flight defaults are saved together.</span><button className="primary-button">Save changes</button></div>
        </form>
      ...
      <main className="u33-settings-workspace">
        ...
        <section className="panel u33-security-panel">
          <div className="security-grid">
            <section><p className="eyebrow">SIGN-IN METHOD</p><h3>Google account</h3><p className="muted">{googleLinked?"Connected. You can use Google to sign in.":"Connect Google after signing in once with your existing password."}</p>{googleLinked?<form action={disconnectGoogle}><button className="secondary-button" disabled={!hasPassword}>Disconnect Google</button>{!hasPassword?<small>Add a password before disconnecting your only sign-in method.</small>:null}</form>:<a className="google-button compact" href="/api/auth/google/start?intent=link"><span aria-hidden="true">G</span>Connect Google</a>}</section>
  ...
  return <>
    ...
    <main className="u33-settings-workspace">
      ...
      <section className="u33-privacy-grid">
        <section className="panel"><p className="eyebrow">PUBLIC SHARING</p><h2>Flight links</h2><p className="muted">{privacy.activePublicShares?`${privacy.activePublicShares} active public flight ${privacy.activePublicShares===1?"link":"links"}.`:"No active public flight links."} Revoked share metadata is automatically removed after 30 days.</p>{privacy.activePublicShares?<form action={revokeAllPublicShares}><button className="secondary-button">Revoke all public links</button></form>:<span className="status-on">No public links</span>}</section>
