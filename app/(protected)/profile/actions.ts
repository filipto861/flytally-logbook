...
import { hashLegacyPassword,verifyLegacyPassword } from "@/lib/auth/password";
...
export async function saveProfile(f:FormData){const {userId}=await requireUser();const name=s(f,"display_name"),email=s(f,"email").toLowerCase();if(!name||!email)return;await sql`UPDATE users SET display_name=${name},email=${email},updated_at=NOW() WHERE id=${userId}`;refresh();}
