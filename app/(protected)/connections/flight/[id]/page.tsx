...
import { approveInstructorFlight,declineInstructorFlight,revokeFlightVerification } from "@/app/(protected)/flights/instructor-actions";
...
export default async function InstructorFlightReview({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<{error?:string}>}){
  ...
  const rows=await sql`SELECT a.id approval_id,a.status,a.record_revision approval_revision,a.flight_hash,a.requested_at,a.decided_at,a.decision_note,p.participant_flight_id,v.id verification_id,v.status verification_status,f.*,u.display_name pilot_name,
  ...
  return <>
    ...
    <section className={`panel instructor-decision-panel ${text(row.status)}`}><div><p className="eyebrow">FLYTALLY VERIFIED APPROVAL</p><h2>{row.status==="approved"?"Flight approved and signed":row.status==="declined"?"Approval declined":current?"Approve this entry?":"Request is no longer current"}</h2><p className="muted">{row.status==="approved"?"Your account, credential snapshot, decision time and server signature are attached to this exact revision.":row.status==="declined"?text(row.decision_note)||"The student can send a new request.":current?"Confirm the displayed record. Your current licence and qualification details (provided by you) will be preserved with a tamper-evident server signature. No extra sign-in is required.":"The student changed or reopened this flight. Ask them to certify the current revision and send a new request."}</p></div>
      {pending&&current?<div className="instructor-decision-actions"><form action={approve}><input type="hidden" name="confirm" value="approve"/><button className="primary-button">Approve & sign</button></form><details><summary className="secondary-button">Decline</summary><form action={decline} className="stack-form"><label>Reason (optional)<textarea name="note" maxLength={500} rows={2}/></label><button className="icon-danger">Decline request</button></form></details></div>:null}
    </section>
    {row.status==="approved"?<section className="panel instructor-approval-panel approved"><div><p className="eyebrow">YOUR LOGBOOK</p><h2>{row.participant_flight_id?"Flight added":"Add your instructor entry"}</h2><p className="muted">{row.participant_flight_id?"Your separate instructor record is ready in your logbook.":"Create your own editable INSTRUCTOR draft with GPS, landings, cost and aircraft data. Private notes stay private."}</p></div>{row.participant_flight_id?<Link className="primary-button" href={`/flights/${text(row.participant_flight_id)}`}>Open my flight</Link>:<form action={add}><button className="primary-button">Add to my logbook</button></form>}</section>:null}
