...
import { acceptConnection,cancelConnection,declineConnection,removeConnection,updateConnection } from "./actions";
...
export default async function ConnectionsPage(){
  ...
  return <>
    ...
    <section className="panel"><div className="section-heading"><div><p className="eyebrow">CONNECTED</p><h2>Your pilots</h2><p className="muted">Relationship and logbook access can be changed anytime.</p></div><span>{connected.length}</span></div>{connected.length?<div className="connection-list">{connected.map(row=>{const requester=Number(row.requester_user_id)===userId,label=text(requester?row.requester_label:row.recipient_label).toLowerCase()||"friend",share=Boolean(requester?row.requester_shares_logbook:row.recipient_shares_logbook),otherShares=Boolean(requester?row.recipient_shares_logbook:row.requester_shares_logbook);return <details className="connection-card connection-manager" key={text(row.id)}><summary><div className="pilot-avatar" aria-hidden="true">{text(row.display_name).slice(0,1).toUpperCase()||"P"}</div><div className="connection-identity"><strong>{text(row.display_name)||"Pilot"}</strong><span>{roleLabel(label)} · {share?"Can view your logbook":"Logbook private"}</span><small>{text(row.home_airport)?`Home airport ${text(row.home_airport)}`:"Home airport not set"}</small></div><span aria-hidden="true">⌄</span></summary><div className="connection-editor"><form action={updateConnection} className="stack-form"><input type="hidden" name="connection_id" value={text(row.id)}/><label>Relationship<select name="relationship_label" defaultValue={label}><option value="friend">Friend</option><option value="student">Student</option><option value="instructor">Instructor</option></select></label><label className="check-row"><input type="checkbox" name="share_logbook" value="yes" defaultChecked={share}/> Allow read-only logbook view</label><small>Price, private notes and GPS stay hidden.</small><button className="primary-button">Save access</button></form><div>{otherShares?<Link className="secondary-button" href={`/connections/logbook/${text(row.id)}`}>View logbook</Link>:null}<details className="confirm-action"><summary>Remove connection…</summary><div><p>Future access and requests stop. Historical signed or accepted flight records remain preserved.</p><form action={removeConnection}><input type="hidden" name="connection_id" value={text(row.id)}/><button className="icon-danger">Remove connection</button></form></div></details></div></div></details>})}</div>:<p className="empty-state">No accepted connections yet.</p>}</section>
    {outgoing.length?<section className="panel"><div className="section-heading"><div><p className="eyebrow">SENT</p><h2>Pending requests</h2></div><span>{outgoing.length}</span></div><div className="connection-list">{outgoing.map(row=><article className="connection-card" key={text(row.id)}><div className="pilot-avatar" aria-hidden="true">{text(row.display_name).slice(0,1).toUpperCase()||"P"}</div><div className="connection-identity"><strong>{text(row.display_name)||"Pilot"}</strong><span>Invited as your {roleLabel(row.requester_label).toLowerCase()}</span><small>Waiting for approval</small></div><form action={cancelConnection}><input type="hidden" name="connection_id" value={text(row.id)}/><button className="secondary-button">Cancel request</button></form></article>)}</div></section>:null}
  </>;
}
