...
const detailSections=new Set(["years","aircraft","airports","routes","recent"]);
const detailHref=(period:string,detail:string)=>`/dashboard?period=${encodeURIComponent(period)}&detail=${encodeURIComponent(detail)}#statistics`;
...
export default async function DashboardPage({searchParams}:{searchParams:Promise<{period?:string;detail?:string}>}){
  const session=await requireUser(),params=await searchParams,requestedPeriod=params.period??"all",selected=periods.some(([key])=>key===requestedPeriod)?requestedPeriod:"all",detail=detailSections.has(params.detail??"")?params.detail??"":"";
  ...
  const renderWidget=(id:DashboardWidgetId)=>{
    ...
    if(id==="airports"){const airport=data.topAirports[0],route=data.topRoutes[0];return <section className="panel dashboard-airports-routes"><div className="section-heading"><div><p className="eyebrow">PLACES & ROUTES</p><h2>Airports & routes</h2></div></div><div className="dashboard-airport-route-summary"><Link href={detailHref(selected,"airports")}><span>Visited airports</span><strong>{data.uniqueAirports}</strong><small>{airport?`Most visited ${airport.airport} · ${airport.visits} visits`:"Open airport history"}</small></Link><Link href={detailHref(selected,"routes")}><span>Flown routes</span><strong>{data.uniqueRoutes}</strong><small>{route?`Top ${route.departure} → ${route.arrival} · ${route.flights} flights`:"Open route history"}</small></Link></div></section>}
    ...
    if(id==="aircraft-costs"){const fleetMinutes=data.topAircraft.reduce((sum,row)=>sum+row.minutes,0),fleetCost=data.topAircraft.reduce((sum,row)=>sum+row.cost,0);return <section className="panel dashboard-aircraft-costs"><div className="section-heading"><div><p className="eyebrow">FLEET & COSTS</p><h2>Aircraft & costs</h2></div><div className="dashboard-heading-actions"><Link className="secondary-link" href={detailHref(selected,"aircraft")}>Full breakdown</Link><Link className="secondary-link" href="/database">Manage aircraft</Link></div></div><div className="dashboard-aircraft-cost-summary"><div><span>Aircraft flown</span><strong>{data.uniqueAircraft}</strong></div><div><span>Cost in period</span><strong>{Math.round(data.cost).toLocaleString("en-GB")} CZK</strong></div><div><span>Average cost / h</span><strong>{fleetMinutes>0?`${Math.round(fleetCost/(fleetMinutes/60)).toLocaleString("en-GB")} CZK/h`:"—"}</strong></div></div>{data.topAircraft.length?<div className="dashboard-fleet-mini">{data.topAircraft.slice(0,3).map(row=><div key={row.registration}><strong>{row.registration}</strong><span>{row.flights} flights · {formatDuration(row.minutes)}</span><b>{Math.round(row.cost).toLocaleString("en-GB")} CZK</b></div>)}</div>:null}</section>}
    ...
    return <DashboardDetails data={data} initialOpen={Boolean(detail)} initialSection={detail||"years"}/>;
  };
  const visibleLayout=layout.filter(item=>item.enabled||(Boolean(detail)&&item.id==="statistics"));
  return <>
    ...
    <div className="period-control">{periods.map(([key,label])=><Link key={key} className={selected===key?'active':''} href={`/dashboard?period=${key}`}>{label}</Link>)}</div>
    <section className="dashboard-layout-grid" aria-label="Dashboard widgets">
      {visibleLayout.map(item=><div id={item.id==="statistics"?"statistics":undefined} key={item.id} data-dashboard-widget={item.id} data-dashboard-size={item.size} className={`dashboard-widget dashboard-size-${item.size}`}>{renderWidget(item.id)}</div>)}
    </section>
