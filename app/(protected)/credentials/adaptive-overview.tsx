...
export async function AdaptivePilotOverview(){
  ...
  return <>
    <header className="page-header"><div><p className="eyebrow">PILOT PROFILE</p><h1>Licences</h1><p className="muted">Your current credentials and flying status. Details stay one tap away.</p></div></header>
    ...
    <section className="adaptive-workspace">
      ...
      {sorted.length?<div className="adaptive-status-list">{sorted.map(item=><Link className="adaptive-status-row" href={item.href} key={item.id}><div className="adaptive-status-copy"><strong>{item.label}</strong><span>{item.detail}</span></div><b className={toneClass(item.tone)}>{item.statusLabel}</b><span className="adaptive-chevron" aria-hidden="true">›</span></Link>)}</div>:<div className="panel adaptive-empty"><strong>No credentials added yet</strong><p className="muted">Add your first licence or document and FlyTally will show only the status that is relevant to you.</p><Link className="button" href="/credentials?view=licences">Add licence</Link></div>}
      <div className="adaptive-detail-links"><Link href="/credentials?view=licences"><strong>Licences & ratings</strong><span>Edit credential details</span></Link><Link href="/credentials?view=recency"><strong>Recency & evidence</strong><span>See calculations and qualifying flights</span></Link><Link href="/credentials?view=training"><strong>Aircraft training</strong><span>Differences and familiarisation evidence</span></Link><Link href="/credentials?view=documents"><strong>Medical & documents</strong><span>Validity and document records</span></Link></div>
    </section>
  </>;
}
