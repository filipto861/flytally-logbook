...
export default async function PrintPage({searchParams}:{searchParams:Promise<Params>}){
  ...
  const sum=(key:string)=>rows.reduce((total,row)=>total+Math.max(0,Number(row[key])||0),0),minutes=sum("block_minutes"),landings=sum("landings_day")+sum("landings_night");
  return <div className="print-logbook easa-print">
    ...
    <table className={styles.table}>
      <colgroup><col className={styles.dateCol}/><col className={styles.placeCol}/><col className={styles.placeCol}/><col className={styles.aircraftCol}/><col className={styles.operationCol}/><col className={styles.flightTimeCol}/><col className={styles.picNameCol}/><col className={styles.functionCol}/><col className={styles.functionCol}/><col className={styles.functionCol}/><col className={styles.functionCol}/><col className={styles.conditionCol}/><col className={styles.landingsCol}/><col className={styles.remarksCol}/></colgroup>
      <thead><tr><th>Date</th><th>Departure<br/><small>place / time</small></th><th>Arrival<br/><small>place / time</small></th><th>Aircraft<br/><small>type / registration</small></th><th>Operation<br/><small>SP/MP · SE/ME</small></th><th>Flight time<br/><small>total / accumulated</small></th><th>Name of PIC</th><th>PIC</th><th>Co-pilot</th><th>Dual</th><th>FI/FE</th><th>Conditions<br/><small>Night / IFR</small></th><th>LDG<br/><small>D / N</small></th><th>Remarks / endorsements / verification</th></tr></thead>
      <tbody>{rows.map((row,index)=>{const verify=verification(row),picName=pilotInCommandName(row,pilotName);return <tr key={`${text(row.date)}-${text(row.registration)}-${text(row.off_block)}-${index}`}><td>{text(row.date)?new Date(`${text(row.date)}T00:00:00`).toLocaleDateString("en-GB"):"—"}</td><td><b>{text(row.departure)||"—"}</b><small>{time(row.off_block)}</small></td><td><b>{text(row.arrival)||"—"}</b><small>{time(row.on_block)}</small></td><td><b>{text(row.aircraft_type)||"—"}</b><small>{text(row.registration)||"—"} · {text(row.aircraft_class)||"—"} · {text(row.evidence)||"—"}</small></td><td>{text(row.operation_type)||"SP"}<small>{text(row.engine_type)||"SE"}</small></td><td><b>{duration(row.block_minutes)}</b><small>Σ {duration(row.accumulated_minutes)}</small></td><td>{picName||"—"}</td><td>{duration(row.pic_minutes)}</td><td>{duration(row.copilot_minutes)}</td><td>{duration(row.dual_minutes)}</td><td>{duration(row.instructor_minutes)}</td><td>{duration(row.night_minutes)}<small>{duration(row.ifr_minutes)}</small></td><td>{text(row.landings_day)||"0"}<small>{text(row.landings_night)||"0"}</small></td><td><b>{text(row.role)||"—"}{text(row.task)?` · ${text(row.task)}`:""}</b>{verify?<small>{verify}</small>:null}{text(row.note)?<small>{text(row.note)}</small>:null}</td></tr>})}</tbody>
    </table>
    ...
    <footer>Generated {new Date().toLocaleDateString("en-GB")} · FlyTally · Electronic flight record structured for AMC1 FCL.050</footer>
  </div>;
}
