...
export async function declineFlightInvitationFromNotification(form:FormData){
  ...
  const notifications=await sql`SELECT href FROM user_notifications WHERE id=${notification} AND user_id=${userId} AND kind IN ('flight_request','flight_invite','signature_request') LIMIT 1` as Array<{href:string}>;
