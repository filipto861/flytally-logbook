...
export default async function NotificationsPage(){
  ...
  return <div className={styles.inbox}>
    <header className="page-header"><div><p className="eyebrow">INBOX</p><h1>Notifications</h1><p className="muted">Updates, completed decisions and reminders. Anything still waiting for your decision is collected in Actions.</p></div><div className="connection-actions"><Link className="secondary-button" href="/actions">Open actions</Link>{unread?<NotificationMutationForm action={markAllNotificationsRead}><button className="secondary-button">Mark all read</button></NotificationMutationForm>:null}{read?<NotificationMutationForm action={clearReadNotifications}><button className="secondary-button">Clear read</button></NotificationMutationForm>:null}</div></header>
    ...
    <section className="panel"><div className="notification-list">{rows.map(row=>{
      ...
      return <article className={`notification-card${row.read_at?"":" unread"}`} key={t(row.id)}><div><span className="eyebrow">{t(row.kind).replaceAll("_"," ")}</span><strong>{t(row.title)}</strong>{row.body?<p className="muted">{t(row.body)}</p>:null}<small>{new Date(t(row.created_at)).toLocaleString("en-GB")}{workflowStatus?` · ${workflowStatus}`:""}</small></div><div className="connection-actions">{pending?<><Link className="primary-button" href={t(row.href)}>{instructor?"Review & sign":"Review & add"}</Link><NotificationMutationForm action={declineFlightInvitationFromNotification}><input type="hidden" name="id" value={t(row.id)}/><button className="secondary-button">Decline</button></NotificationMutationForm></>:row.href?<Link className="primary-button" href={t(row.href)}>Open</Link>:null}{!row.read_at&&!pending?<NotificationMutationForm action={markNotificationRead}><input type="hidden" name="id" value={t(row.id)}/><button className="secondary-button">Mark read</button></NotificationMutationForm>:null}<NotificationMutationForm action={deleteNotification}><input type="hidden" name="id" value={t(row.id)}/><button className="secondary-button">Delete</button></NotificationMutationForm></div></article>})}{!rows.length?<p className="empty-state">No notifications yet.</p>:null}</div></section>
  </div>;
}
