...
export default async function NotificationsPage(){
  ...
  return <>
    ...
    <section className="panel"><div className="notification-list">{rows.map(row=>{
      const requestKind=["flight_request","flight_invite"].includes(t(row.kind))&&Boolean(row.participation_id),status=t(row.request_status).toLowerCase(),pending=requestKind&&status==="pending",instructor=t(row.participant_role).toUpperCase()==="INSTRUCTOR";
