...
import { createManualBackup,restorePortableBackup,restoreStoredBackup } from "./actions";
import { ensureDailyBackup,listStoredBackups } from "@/lib/backup-center";

export const metadata={title:"Export | FlyTally"};

export default async function ExportPage(){
  const {userId}=await requireUser();await ensureDailyBackup(userId);const [d,backups]=await Promise.all([getDashboardData(userId,"all"),listStoredBackups(userId)]);
  return <>
    ...
    <BackupCenter backups={backups} createAction={createManualBackup} restoreAction={restoreStoredBackup}/><BackupValidator/><BackupRestore action={restorePortableBackup}/>
