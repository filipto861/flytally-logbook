...
export default async function NewFlightPage({searchParams}:{searchParams:Promise<{mode?:string;added?:string}>}) {
  ...
  const [aircraft,routes,defaults,instructorRows]=await Promise.all([
    getAircraftOptions(userId),getRecentRoutes(userId),getManualEntryDefaults(userId),
