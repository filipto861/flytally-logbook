...
async function materializeParticipation(participationId:number,userId:number){
  const rows=await sql`SELECT p.id participation_id,p.source_flight_id,p.source_user_id,p.participant_user_id,p.participant_role,p.source_revision,p.source_hash,p.status,p.participant_flight_id,
    f.date::text date,f.evidence,f.registration,f.aircraft_type,f.aircraft_class,f.regulatory_category,f.balloon_class,f.balloon_group,f.launch_method,f.launches,f.aircraft_make,f.aircraft_model,f.aircraft_variant,f.departure,f.arrival,f.off_block,f.takeoff,f.landing,f.on_block,
    f.task,f.purpose_code,f.operation_type,f.engine_type,f.starts,f.landings_day,f.landings_night,f.movement_evidence_recorded,f.takeoffs_day,f.takeoffs_night,f.approaches_day,f.approaches_night,f.night_minutes,f.ifr_minutes,f.price_per_hour,f.billing_basis,f.certified_at,f.certification_hash,f.record_revision,
  ...
  const role=participantLogbookRole(participantRole),block=flightMinutes(text(row.off_block),text(row.on_block)),air=flightMinutes(text(row.takeoff),text(row.landing)),creditMinutes=["SAILPLANE","BALLOON"].includes(text(row.regulatory_category).toUpperCase())?(air||block):block,credit=crewRoleCredits(participantRole,creditMinutes),instructor=participantRole==="INSTRUCTOR",commander=instructor?text(row.participant_name):text(row.source_name),instructorName="";
  ...
  const result=await sql`WITH locked AS MATERIALIZED(SELECT pg_advisory_xact_lock(hashtextextended(${fingerprint},0))),
    ...
    aircraft_profile AS(INSERT INTO aircraft(user_id,registration,aircraft_type,aircraft_make,aircraft_model,aircraft_variant,icao_type,aircraft_class,regulatory_category,balloon_class,balloon_group,evidence,default_price_per_hour,default_role,billing_basis,active,note,created_at,updated_at)
      SELECT ${userId},${text(row.registration)},${text(row.aircraft_type)},${text(row.aircraft_make)},${text(row.aircraft_model)},${text(row.aircraft_variant)},${text(row.icao_type)},${text(row.aircraft_class)},${text(row.regulatory_category)},${text(row.balloon_class)},${text(row.balloon_group)},${text(row.evidence)},${row.price_per_hour===null?null:Number(row.price_per_hour)||0},${role},${text(row.billing_basis)||'BLOCK'},1,'Added from a shared certified flight',NOW(),NOW()
    ...
    inserted AS(INSERT INTO flights(user_id,date,evidence,registration,aircraft_type,aircraft_class,regulatory_category,balloon_class,balloon_group,launch_method,launches,aircraft_make,aircraft_model,aircraft_variant,departure,arrival,off_block,takeoff,landing,on_block,starts,commander,instructor,role,task,purpose_code,price_per_hour,billing_basis,note,operation_type,engine_type,landings_day,landings_night,movement_evidence_recorded,takeoffs_day,takeoffs_night,approaches_day,approaches_night,night_minutes,ifr_minutes,pic_minutes,copilot_minutes,dual_minutes,instructor_minutes,verification_name,verification_reference)
      SELECT ${userId},${text(row.date)},${text(row.evidence)},${text(row.registration)},${text(row.aircraft_type)},${text(row.aircraft_class)},${text(row.regulatory_category)},${text(row.balloon_class)},${text(row.balloon_group)},${text(row.launch_method)},${Number(row.launches)||0},${text(row.aircraft_make)},${text(row.aircraft_model)},${text(row.aircraft_variant)},${text(row.departure)},${text(row.arrival)},${text(row.off_block)},${text(row.takeoff)},${text(row.landing)},${text(row.on_block)},${Number(row.starts)||0},${commander},${instructorName},${role},${text(row.task)},${text(row.purpose_code)},${row.price_per_hour===null?null:Number(row.price_per_hour)||0},${text(row.billing_basis)||'BLOCK'},${instructor?`FI entry linked to ${text(row.source_name)}'s verified training flight`:`Shared flight with ${text(row.source_name)}`},${text(row.operation_type)||"SP"},${text(row.engine_type)||"SE"},${Number(row.landings_day)||0},${Number(row.landings_night)||0},${Boolean(row.movement_evidence_recorded)},${Number(row.takeoffs_day)||0},${Number(row.takeoffs_night)||0},${Number(row.approaches_day)||0},${Number(row.approaches_night)||0},${Number(row.night_minutes)||0},${Number(row.ifr_minutes)||0},${credit.pic},${credit.copilot},0,${credit.instructor},'',''
