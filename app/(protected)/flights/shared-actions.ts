...
async function materializeParticipation(participationId:number,userId:number){
  const rows=await sql`SELECT p.id participation_id,p.source_flight_id,p.source_user_id,p.participant_user_id,p.participant_role,p.source_revision,p.source_hash,p.status,p.participant_flight_id,
    f.date::text date,f.evidence,f.registration,f.aircraft_type,f.aircraft_class,f.regulatory_category,f.launch_method,f.launches,f.aircraft_make,f.aircraft_model,f.aircraft_variant,f.departure,f.arrival,f.off_block,f.takeoff,f.landing,f.on_block,
  ...
  const role=participantLogbookRole(participantRole),block=flightMinutes(text(row.off_block),text(row.on_block)),air=flightMinutes(text(row.takeoff),text(row.landing)),creditMinutes=text(row.regulatory_category).toUpperCase()==="SAILPLANE"?(air||block):block,credit=crewRoleCredits(participantRole,creditMinutes),instructor=participantRole==="INSTRUCTOR",commander=instructor?text(row.participant_name):text(row.source_name),instructorName="";
  ...
  const result=await sql`WITH locked AS MATERIALIZED(SELECT pg_advisory_xact_lock(hashtextextended(${fingerprint},0))),
    ...
    aircraft_profile AS(INSERT INTO aircraft(user_id,registration,aircraft_type,aircraft_make,aircraft_model,aircraft_variant,icao_type,aircraft_class,regulatory_category,evidence,default_price_per_hour,default_role,billing_basis,active,note,created_at,updated_at)
      SELECT ${userId},${text(row.registration)},${text(row.aircraft_type)},${text(row.aircraft_make)},${text(row.aircraft_model)},${text(row.aircraft_variant)},${text(row.icao_type)},${text(row.aircraft_class)},${text(row.regulatory_category)},${text(row.evidence)},${row.price_per_hour===null?null:Number(row.price_per_hour)||0},${role},${text(row.billing_basis)||'BLOCK'},1,'Added from a shared certified flight',NOW(),NOW()
