...
export async function requestInstructorApproval(flightId:number,form:FormData){
  ...
  const rows=await sql`INSERT INTO instructor_flight_approvals(flight_id,student_user_id,instructor_user_id,record_revision,flight_hash,status)
    ...
    FROM flights f WHERE f.id=${flightId} AND f.user_id=${userId} AND f.certified_at IS NOT NULL
      ...
      AND EXISTS(SELECT 1 FROM pilot_connections c WHERE c.status='accepted' AND
        ((c.requester_user_id=${instructorId} AND c.recipient_user_id=${userId} AND (c.requester_label='instructor' OR c.relationship='requester_instructor')) OR
         (c.recipient_user_id=${instructorId} AND c.requester_user_id=${userId} AND (c.recipient_label='instructor' OR c.relationship='recipient_instructor'))))
  ...
  if(rows[0])await notifyUser(instructorId,{kind:"signature_request",title:"Flight approval requested",body:"Review and sign the exact certified revision.",href:`/connections/flight/${rows[0].id}`,dedupeKey:`approval:${rows[0].id}`});
  refresh(flightId);
}
...
async function decide(approvalId:number,status:"approved"|"declined",form:FormData){
  ...
  const rows=await sql`UPDATE instructor_flight_approvals a SET status=${status},decided_at=NOW(),decision_note=${note}
    FROM flights f WHERE a.id=${approvalId} AND a.flight_id=f.id AND a.instructor_user_id=${userId} AND a.status='pending'
      ...
      AND EXISTS(SELECT 1 FROM pilot_connections c WHERE c.status='accepted' AND
        ((c.requester_user_id=${userId} AND c.recipient_user_id=a.student_user_id AND (c.requester_label='instructor' OR c.relationship='requester_instructor')) OR
         (c.recipient_user_id=${userId} AND c.requester_user_id=a.student_user_id AND (c.recipient_label='instructor' OR c.relationship='recipient_instructor'))))
