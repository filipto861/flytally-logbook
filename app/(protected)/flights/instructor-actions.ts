...
import { cancelInstructorRequest,upsertInstructorRequest } from "@/lib/training-verification";
...
const refresh=(flightId:number,approvalId?:number)=>{
  revalidatePath(`/flights/${flightId}`);revalidatePath(`/flights/${flightId}/audit`);revalidatePath("/connections");revalidatePath("/print");revalidatePath("/notifications");revalidatePath("/credentials");revalidatePath("/dashboard");
...
export async function requestInstructorApproval(flightId:number,form:FormData){
  ...
  const instructorId=id(form.get("instructor_id"));if(!Number.isSafeInteger(flightId)||flightId<=0||!instructorId||instructorId===userId)return;
  await upsertInstructorRequest(userId,flightId,instructorId);refresh(flightId);
}
...
async function decide(approvalId:number,status:"approved"|"declined",form:FormData){
  ...
  const rows=await sql`UPDATE instructor_flight_approvals a SET status=${status},decided_at=NOW(),decision_note=${note}
    FROM flights f WHERE a.id=${approvalId} AND a.flight_id=f.id AND a.instructor_user_id=${userId} AND a.status='pending'
      AND f.user_id=a.student_user_id AND f.certified_at IS NOT NULL AND COALESCE(f.record_revision,1)=a.record_revision AND f.certification_hash=a.flight_hash
    ...
    RETURNING a.flight_id,a.student_user_id,a.record_revision` as Array<{flight_id:number|string;student_user_id:number|string;record_revision:number|string}>;
  const row=rows[0];
  await sql`UPDATE user_notifications SET read_at=COALESCE(read_at,NOW()) WHERE user_id=${userId} AND (href=${`/connections/flight/${approvalId}`} OR href IN (SELECT '/connections/shared/'||p.id FROM flight_participations p WHERE p.approval_id=${approvalId} AND p.participant_user_id=${userId}))`;
  if(row){
    await sql`UPDATE flight_participations SET status=${status==="approved"?"accepted":"declined"},responded_at=NOW(),decision_note=${note}
      WHERE approval_id=${approvalId} AND participant_user_id=${userId} AND source_flight_id=${Number(row.flight_id)} AND source_revision=${Number(row.record_revision)} AND status='pending'`;
    await notifyUser(Number(row.student_user_id),{kind:`approval_${status}`,title:status==="approved"?"Flight approved":"Flight approval declined",body:note,href:`/flights/${row.flight_id}`,dedupeKey:`approval-decision:${approvalId}:${status}`});refresh(Number(row.flight_id),approvalId);
  }
  return row;
}
...
export async function approveInstructorFlight(approvalId:number,form:FormData){
  ...
  const enabled=await sql`SELECT enabled FROM feature_switches WHERE key='verified_approvals' LIMIT 1` as Array<{enabled:boolean}>;if(enabled[0]&&!enabled[0].enabled)return;
  const decided=await decide(approvalId,"approved",form);if(!decided)return;
  ...
  const credentialSnapshot={identity:String(row.display_name),source:"FlyTally account",licences,qualifications},verificationRole=String(row.role).toUpperCase()==="DUAL"?"INSTRUCTOR":"SUPERVISING PIC";
  ...
  await sql`UPDATE flight_participations SET status='accepted',responded_at=COALESCE(responded_at,NOW()) WHERE approval_id=${approvalId} AND participant_user_id=${session.userId}`;
  ...
  refresh(payload.flightId,approvalId);
}

export async function approveAndAddInstructorFlight(approvalId:number,form:FormData){form.set("confirm","approve");await approveInstructorFlight(approvalId,form);return addApprovedFlightToLogbook(approvalId);}

export async function revokeFlightVerification(form:FormData){const session=await requireUser(),verificationId=id(form.get("verification_id")),reason=text(form.get("reason")).slice(0,500);if(!verificationId||reason.length<5)return;const rows=await sql`UPDATE flight_verifications SET status='revoked',revoked_at=NOW(),revocation_reason=${reason} WHERE id=${verificationId} AND signer_user_id=${session.userId} AND status='signed' RETURNING flight_id,flight_user_id` as Array<{flight_id:number|string;flight_user_id:number|string}>;if(rows[0]){await notifyUser(Number(rows[0].flight_user_id),{kind:"signature_revoked",title:"Flight approval revoked",body:reason,href:`/flights/${rows[0].flight_id}/audit`,dedupeKey:`verification-revoked:${verificationId}`});await refreshRecencySnapshot(Number(rows[0].flight_user_id));refresh(Number(rows[0].flight_id))}}
export async function declineInstructorFlight(approvalId:number,form:FormData){await decide(approvalId,"declined",form);}
export async function cancelInstructorApproval(flightId:number,form:FormData){
  const{userId}=await requireUser();await ensureV132Schema();const approvalId=id(form.get("approval_id")),participationId=id(form.get("participation_id"));
  if(participationId){await cancelInstructorRequest(userId,flightId,participationId);refresh(flightId);return}
  if(!approvalId)return;
  const p=await sql`SELECT id FROM flight_participations WHERE approval_id=${approvalId} AND source_flight_id=${flightId} AND source_user_id=${userId} AND status='pending' LIMIT 1` as Array<{id:number|string}>;
  if(p[0])await cancelInstructorRequest(userId,flightId,Number(p[0].id));else await sql`UPDATE instructor_flight_approvals SET status='cancelled',decided_at=NOW(),decision_note='Cancelled by pilot.' WHERE id=${approvalId} AND flight_id=${flightId} AND student_user_id=${userId} AND status='pending'`;
  refresh(flightId,approvalId);
}
