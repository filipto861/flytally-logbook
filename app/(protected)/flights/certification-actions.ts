...
export async function certifyFlight(flightId:number,form:FormData){
  ...
  const rows=await sql`SELECT f.id,f.date::text date,f.evidence,f.registration,f.aircraft_make,f.aircraft_model,f.aircraft_variant,f.aircraft_type,f.aircraft_class,f.departure,f.arrival,f.off_block,f.takeoff,f.landing,f.on_block,f.operation_type,f.engine_type,f.landings_day,f.landings_night,f.night_minutes,f.ifr_minutes,f.pic_minutes,f.copilot_minutes,f.dual_minutes,f.instructor_minutes,f.commander,f.instructor,f.role,f.task,f.note,f.verification_name,f.verification_reference,f.certified_at,f.certification_version,f.record_revision,f.correction_reason,u.display_name pilot_name FROM flights f JOIN users u ON u.id=f.user_id WHERE f.id=${flightId} AND f.user_id=${userId} LIMIT 1` as Array<Record<string,unknown>>;
