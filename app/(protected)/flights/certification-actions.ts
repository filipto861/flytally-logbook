...
export async function certifyFlight(flightId:number,form:FormData){
  ...
  const certificationHash=flightCertificationHash({...row,certification_version:6},userId,6);
  await sql.transaction([
    ...
    sql`UPDATE flights SET certified_at=NOW(),certified_by_user_id=${userId},certification_hash=${certificationHash},certification_version=6,locked_at=NOW(),locked_by_user_id=${userId} WHERE id=${flightId} AND user_id=${userId} AND certified_at IS NULL`,
  ]);
