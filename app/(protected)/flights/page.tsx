...
export default async function FlightsPage({searchParams}:{searchParams:Promise<Params>}){
  ...
  return <>
    ...
    <section className="table-panel"><div className="table-scroll"><table><thead><tr><th>Detail</th><th>Datum</th><th>Letadlo</th><th>Trasa</th><th>Časy</th><th>BLOCK</th><th>Funkce</th><th>Přist.</th><th>GPS</th></tr></thead><tbody>{result.rows.map(f=><tr key={f.id}><td><Link className="detail-button" href={detailHref(f.id,params)}>Otevřít</Link></td><td>{new Date(`${f.date}T00:00:00`).toLocaleDateString("cs-CZ")}</td><td><strong>{f.registration||"—"}</strong><small>{f.aircraft_type||f.evidence}</small></td><td><strong>{f.departure||"—"} → {f.arrival||"—"}</strong><small>{f.task||""}</small></td><td>{f.off_block||"—"}–{f.on_block||"—"}<small>{f.takeoff&&f.landing?`${f.takeoff}–${f.landing}`:""}</small></td><td><strong>{formatDuration(f.block_minutes)}</strong><small>AIR {formatDuration(f.air_minutes)}</small></td><td>{f.role||"—"}<small>{f.evidence}</small></td><td>{f.starts}</td><td>{f.track_count?<Link className="gps-badge" href={detailHref(f.id,params)}>● {f.track_count}<small>{f.gps_km.toFixed(0)} km</small></Link>:<span className="muted">—</span>}</td></tr>)}{!result.rows.length?<tr><td colSpan={9} className="empty-state">Filtrům neodpovídá žádný let.</td></tr>:null}</tbody></table></div>
