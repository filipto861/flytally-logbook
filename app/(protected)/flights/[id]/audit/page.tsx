...
export default async function FlightAuditReport({params}:{params:Promise<{id:string}>}){
  ...
  return <div className="print-logbook">
    ...
    {approvalRows.length?<section className="panel"><p className="eyebrow">INSTRUCTOR APPROVAL</p><h2>Instructor decision history</h2><div className="flight-audit-list">{approvalRows.map(row=><article key={text(row.record_revision)}><header><div><strong>Revision {text(row.record_revision)} · {text(row.instructor_name)}</strong><small>Requested {dateTime(row.requested_at)}{row.decided_at?` · Decided ${dateTime(row.decided_at)}`:""}</small></div><span className={`audit-action ${row.status==="approved"?"created":row.status==="declined"?"deleted":"updated"}`}>{text(row.status)}</span></header>{row.decision_note?<p>{text(row.decision_note)}</p>:null}</article>)}</div></section>:null}
    {verificationRows.length?<section className="panel"><p className="eyebrow">VERIFIED APPROVALS</p><h2>Signature evidence</h2><div className="flight-audit-list">{verificationRows.map(row=><article key={text(row.id)}><header><div><strong>R{text(row.record_revision)} · {text(row.signer_name)||"Deleted account"} · {text(row.verification_role)}</strong><small>{row.signed_at?`Signed ${dateTime(row.signed_at)}`:"Not signed"}{row.revoked_at?` · Revoked ${dateTime(row.revoked_at)}`:""}</small></div><span className={`audit-action ${row.status==="signed"?"created":"deleted"}`}>{text(row.status)}</span></header><p className="muted">Credential source: Provided by pilot · payload SHA-256 {text(row.payload_hash)||"—"}</p><p className="muted">Server signature {text(row.server_signature)||"—"}</p>{row.revocation_reason?<p><b>Revocation reason:</b> {text(row.revocation_reason)}</p>:null}</article>)}</div></section>:null}
