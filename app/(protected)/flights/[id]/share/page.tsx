...
export default async function ShareFlightPage({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<{token?:string}>}){
  ...
  if(!f.certified_at)return <div className="ui-page-stack">
    <header className="page-header"><div><p className="eyebrow">SHARE FLIGHT</p><h1>{String(f.departure||"—")} → {String(f.arrival||"—")}</h1><p className="muted">Sharing becomes available after the flight record is certified.</p></div><Link className="secondary-button" href={`/flights/${id}`}>Back to flight</Link></header>
  ...
  return <div className="ui-page-stack">
    <header className="page-header"><div><p className="eyebrow">SHARE CERTIFIED R{Number(f.record_revision)||1}</p><h1>{String(f.departure||"—")} → {String(f.arrival||"—")}</h1><p className="muted">Share a deliberately limited public view of the protected record. Logbook remarks, costs, signatures, licence data, certification and crew data are never included.</p></div><Link className="secondary-button" href={`/flights/${id}`}>Back to flight</Link></header>
