...
import { requestInstructorApproval } from "../instructor-actions";
import { cancelSafetyInvitation,inviteSafetyPilot } from "../shared-actions";
...
export default async function FlightDetailPage({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<Context>}){
  ...
  const [flight,aircraft,tracks,navigation,instructors,approvalRows,crewOptions,safetyRows]=await Promise.all([
    ...
    sql`SELECT u.id,u.display_name FROM pilot_connections c JOIN users u ON u.id=CASE WHEN c.relationship='requester_instructor' THEN c.requester_user_id ELSE c.recipient_user_id END
      WHERE c.status='accepted' AND ((c.relationship='requester_instructor' AND c.recipient_user_id=${userId}) OR (c.relationship='recipient_instructor' AND c.requester_user_id=${userId})) ORDER BY u.display_name` as Promise<Array<Record<string,unknown>>>,
    ...
    sql`SELECT p.id,p.status,p.participant_flight_id,u.display_name participant_name FROM flight_participations p JOIN users u ON u.id=p.participant_user_id JOIN flights f ON f.id=p.source_flight_id WHERE p.source_flight_id=${id} AND p.source_user_id=${userId} AND p.participant_role='SAFETY PILOT' AND p.source_revision=COALESCE(f.record_revision,1) LIMIT 1` as Promise<Array<Record<string,unknown>>>,
  ]);
  if(!flight)notFound();
  ...
  const role=String(flight.role??"").trim().toUpperCase(),approval=approvalRows[0],trainingRole=["DUAL","SPIC","PICUS"].includes(role),requestApproval=requestInstructorApproval.bind(null,id);
  const approvalPanel=trainingRole?<section className={`panel instructor-approval-panel ${approval?.status??""}`}><div><p className="eyebrow">INSTRUCTOR APPROVAL</p><h2>{approval?.status==="approved"?"Approved by instructor":approval?.status==="pending"?"Waiting for instructor":approval?.status==="declined"?"Approval declined":"Instructor confirmation"}</h2><p className="muted">{approval?.status==="approved"?`${String(approval.instructor_name)} approved this exact certified revision.`:approval?.status==="pending"?`Request sent to ${String(approval.instructor_name)}. You do not need to do anything else.`:approval?.status==="declined"?`${String(approval.instructor_name)} declined the request${approval.decision_note?`: ${String(approval.decision_note)}`:"."}`:certified?"Send this certified training entry to your connected instructor.":"Certify the final flight record first. Then you can request instructor approval here."}</p></div>{certified&&(!approval||approval.status==="declined")?<>{instructors.length?<form action={requestApproval} className="instructor-request-form">{instructors.length===1?<input type="hidden" name="instructor_id" value={String(instructors[0].id)}/>:<label>Instructor<select name="instructor_id" required>{instructors.map(row=><option value={String(row.id)} key={String(row.id)}>{String(row.display_name)}</option>)}</select></label>}<button className="primary-button">Request approval{instructors.length===1?` from ${String(instructors[0].display_name)}`:""}</button></form>:<Link className="secondary-button" href="/connections">Connect an instructor</Link>}</>:null}</section>:null;
  const safety=safetyRows[0],inviteSafety=inviteSafetyPilot.bind(null,id),cancelSafety=cancelSafetyInvitation.bind(null,id);
  const safetyPanel=certified&&role==="PIC"?<section className={`panel instructor-approval-panel ${safety?.status??""}`}><div><p className="eyebrow">SHARE THIS FLIGHT</p><h2>{safety?.status==="accepted"?"Added by Safety Pilot":safety?.status==="pending"?"Safety Pilot invited":safety?.status==="declined"?"Invitation declined":"Invite a Safety Pilot"}</h2><p className="muted">{safety?.status==="accepted"?`${String(safety.participant_name)} added a separate Safety Pilot entry to their logbook.`:safety?.status==="pending"?`Waiting for ${String(safety.participant_name)} to accept the flight.`:safety?.status==="declined"?`${String(safety.participant_name)} declined the invitation.`:"The selected pilot will review the flight before anything is added to their logbook."}</p></div>{!safety||safety.status==="declined"?<>{crewOptions.length?<form action={inviteSafety} className="instructor-request-form">{crewOptions.length===1?<input type="hidden" name="participant_id" value={String(crewOptions[0].id)}/>:<label>Safety Pilot<select name="participant_id" required>{crewOptions.map(row=><option value={String(row.id)} key={String(row.id)}>{String(row.display_name)}</option>)}</select></label>}<button className="primary-button">Invite Safety Pilot</button></form>:<Link className="secondary-button" href="/connections">Connect a pilot</Link>}</>:safety.status==="pending"?<form action={cancelSafety}><input type="hidden" name="participation_id" value={String(safety.id)}/><button className="secondary-button">Cancel invitation</button></form>:null}</section>:null;
  const overview=<>
