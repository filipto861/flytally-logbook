...
export default async function FlightDetailPage({ params,searchParams }: { params: Promise<{ id: string }>;searchParams:Promise<Context> }) {
  ...
  const compliance=fcl050FlightCompliance({...flight,...certification},String(certification.pilot_name??"")),blockers=blockingComplianceIssues(compliance),easa=String(flight.evidence??"").trim().toUpperCase()==="EASA";
  ...
  {revisionRows.length||certified?<details className="panel certified-revision-history"><summary>Revision history</summary><div className="flight-audit-list">{certified?<article><header><div><strong>R{recordRevision} · Current certified version</strong><small>{dateTime(certification.certified_at)}</small></div></header></article>:<article><header><div><strong>R{recordRevision} · Correction draft</strong><small>{certification.correction_opened_at?dateTime(certification.correction_opened_at):"Opened for correction"}</small></div></header>{correctionReason?<p className="muted">{correctionReason}</p>:null}</article>}{revisionRows.map(row=><article key={String(row.revision_number)}><header><div><strong>R{String(row.revision_number)} · Superseded</strong><small>{dateTime(row.certified_at)}</small></div></header>{row.correction_reason?<p className="muted">{String(row.correction_reason)}</p>:null}</article>)}</div></details>:null}

  {tracks.length?<>{!locked?<AirportDetectionControl action={detect} departure={flight.departure} arrival={flight.arrival}/>:null}<FlightTrackPlayer tracks={tracks}/></>:<section className="panel no-track"><p className="eyebrow">GPS</p><h2>No GPS track</h2></section>}{!locked?<TrackManager flightId={id} tracks={tracks} attachAction={attach} applyAction={apply} deleteAction={dropTrack}/>:null}{!locked?<details className="panel edit-flight" open><summary>Flight details</summary><FlightForm key={`${flight.id}:${flight.registration}:${flight.departure}:${flight.arrival}:${recordRevision}`} action={update} aircraft={aircraft} initial={flight}/></details>:<section className="panel locked-flight-note"><strong>{certified?"Editing is disabled for this certified revision.":"Editing is disabled while this flight is locked."}</strong><p>{certified?"Use Correct flight above if a change is needed. The current revision stays in the audit history.":"Unlock the flight if a correction is required."}</p></section>}<FlightAuditPanel events={audit}/></>;
}
