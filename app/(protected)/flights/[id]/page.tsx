...
export default async function FlightDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [flight, aircraft, tracks,navigation] = await Promise.all([getFlight(userId, id), getAircraftOptions(userId), getFlightTracks(userId, id),getFlightNavigation(userId,id)]);
  ...
  return <><header className="page-header"><div><p className="eyebrow">DETAIL LETU · {navigation.position}/{navigation.total}</p><h1>{flight.registration} · {flight.date}</h1><p className="muted">{flight.departure} → {flight.arrival} · {flight.role} · {flight.evidence}</p></div><div className="detail-navigation">{navigation.previousId?<Link className="secondary-link" href={`/flights/${navigation.previousId}`}>← Novější</Link>:null}{navigation.nextId?<Link className="secondary-link" href={`/flights/${navigation.nextId}`}>Starší →</Link>:null}<form action={remove}><button className="danger-button">Smazat let</button></form></div></header><section className="flight-summary detail-summary"><div><span>BLOCK</span><strong>{formatDuration(flight.block_minutes)}</strong><small>{flight.off_block}–{flight.on_block}</small></div><div><span>AIR</span><strong>{formatDuration(flight.air_minutes)}</strong><small>{flight.takeoff}–{flight.landing}</small></div><div><span>Přistání</span><strong>{flight.starts}</strong><small>{flight.task||'let'}</small></div><div><span>GPS</span><strong>{flight.gps_km.toFixed(1)} km</strong><small>{flight.track_count} tracků</small></div></section>{tracks.length?<FlightTrackPlayer tracks={tracks}/>:<section className="panel no-track"><p className="eyebrow">GPS</p><h2>K tomuto letu není uložený track</h2><p className="muted">Níže lze GPS track přidat bez vytváření nového letu.</p></section>}<TrackManager flightId={id} tracks={tracks} attachAction={attach} applyAction={apply} deleteAction={dropTrack}/><details className="panel edit-flight" open><summary>Údaje letu a editace</summary><FlightForm action={update} aircraft={aircraft} initial={flight}/></details></>;
}
