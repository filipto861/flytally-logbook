...
export default async function FlightDetailPage({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<Context>}){
  ...
  const [flight,aircraft,tracks,navigation,instructors,approvalRows]=await Promise.all([
    getFlight(userId,id),
  ...
  const approval=approvalRows[0],trainingRole=["DUAL","SPIC","PICUS"].includes(String(flight.role??"").trim().toUpperCase()),requestApproval=requestInstructorApproval.bind(null,id);
