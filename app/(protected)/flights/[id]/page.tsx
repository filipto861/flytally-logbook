...
import { FlightGpsFallback,FlightGpsSection } from "@/components/flight-gps-section";
...
export default async function FlightDetailPage({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<Context>}){
  const {userId}=await requireUser(),id=Number((await params).id),context=await searchParams,query=contextQuery(context),suffix=query?`?${query}`:"";
  if(!Number.isSafeInteger(id)||id<=0)notFound();
  await ensureDatabaseOptimizations();
  const [flight,aircraft,navigation]=await Promise.all([getFlight(userId,id),getAircraftOptions(userId),getFlightNavigation(userId,id,context)]);
  if(!flight)notFound();
  ...
  const hasCertifiedHistory=certified||recordRevision>1||Boolean(correctionReason),toggleLock=setFlightLock.bind(null,id),locked=Boolean(flight.locked_at)||certified;
  ...
  return <>
    ...
    <Suspense fallback={<FlightGpsFallback/>}><FlightGpsSection userId={userId} flightId={id} locked={locked} departure={flight.departure} arrival={flight.arrival}/></Suspense>
