...
export default async function FlightDetailPage({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<Context>}){
  ...
  const role=String(flight.role??"").trim().toUpperCase(),approval=approvalRows[0],trainingRole=["DUAL","SPIC","PICUS"].includes(role),requestApproval=requestInstructorApproval.bind(null,id),cancelApproval=cancelInstructorApproval.bind(null,id);
  const approvalPanel=trainingRole?<section className={`panel instructor-approval-panel ${approval?.status??""}`}><div><p className="eyebrow">CREW VERIFICATION</p><h2>{approval?.status==="approved"?"Approved by instructor":approval?.status==="pending"?"Waiting for instructor":approval?.status==="declined"?"Approval declined":"Instructor confirmation"}</h2><p className="muted">{approval?.status==="approved"?`${String(approval.instructor_name)} approved this exact certified revision.`:approval?.status==="pending"?`Request sent automatically to ${String(approval.instructor_name)}. They can review, sign and add their own logbook entry.`:approval?.status==="declined"?`${String(approval.instructor_name)} declined the request${approval.decision_note?`: ${String(approval.decision_note)}`:"."}`:certified?"Use a connected FlyTally instructor, or let an instructor without FlyTally sign this certified revision directly on this device.":"Choose the actual instructor or supervising PIC in Logbook data. FlyTally will send the certified revision automatically when the name matches a connected instructor."}</p></div><div className="record-protection-actions">{approval?.status==="pending"?<form action={cancelApproval}><input type="hidden" name="approval_id" value={String(approval.id)}/><button className="secondary-button">Cancel request</button></form>:certified&&(!approval||["declined","cancelled","superseded"].includes(String(approval.status)))?<>{instructors.length?<form action={requestApproval} className="instructor-request-form">{instructors.length===1?<input type="hidden" name="instructor_id" value={String(instructors[0].id)}/>:<label>Instructor<select name="instructor_id" required>{instructors.map(row=><option value={String(row.id)} key={String(row.id)}>{String(row.display_name)}</option>)}</select></label>}<button className="primary-button">Request approval{instructors.length===1?` from ${String(instructors[0].display_name)}`:""}</button></form>:<Link className="secondary-button" href="/connections">Connect an instructor</Link>}</>:null}{certified&&approval?.status!=="approved"?<Link className="secondary-button" href={`/flights/${id}/in-person-signature`}>Instructor without FlyTally · Sign on this device</Link>:null}</div></section>:null;
