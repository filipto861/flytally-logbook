...
export async function importKmlFlight(_:FlightActionState,form:FormData):Promise<FlightActionState>{
  const {userId}=await requireUser();await ensureDatabaseOptimizations();const file=form.get("kml");if(!(file instanceof File)||!file.size)return {error:"Select a KML, GPX or CSV file."};if(file.size>8*1024*1024)return {error:"File exceeds 8 MB."};
