...
export async function createFlight(_: FlightActionState, form: FormData): Promise<FlightActionState> {
  ...
  const results=await sql.transaction([
    ...
    sql`INSERT INTO flights (user_id,date,evidence,registration,aircraft_type,aircraft_class,departure,arrival,off_block,takeoff,landing,on_block,starts,commander,instructor,role,task,price_per_hour,billing_basis,note,operation_type,engine_type,landings_day,landings_night,night_minutes,ifr_minutes,pic_minutes,copilot_minutes,dual_minutes,instructor_minutes,verification_name,verification_reference)
      SELECT ${userId},${f.date},${f.evidence},${f.registration},${f.aircraftType},${f.aircraftClass},${departure},${arrival},${f.offBlock},${f.takeoff},${f.landing},${f.onBlock},${f.starts},${f.commander},${f.instructor},${f.role},${f.task},${price},${f.billingBasis},${f.note},${f.operationType},${f.engineType},${f.landingsDay},${f.landingsNight},${f.nightMinutes},${f.ifrMinutes},${f.picMinutes},${f.copilotMinutes},${f.dualMinutes},${f.instructorMinutes},${f.verificationName},${f.verificationReference}
...
export async function importKmlFlight(_:FlightActionState,form:FormData):Promise<FlightActionState>{
  const {userId}=await requireUser();const file=form.get("kml");if(!(file instanceof File)||!file.size)return {error:"Select a KML, GPX or CSV file."};if(file.size>8*1024*1024)return {error:"File exceeds 8 MB."};
