...
import { allocatedFunctionTimes,defaultEngineType } from "@/lib/easa-logbook";
...
export type AirportCandidate={ident:string;name:string;distanceKm:number;confidence:"high"|"medium"|"low"|"manual";source:"custom"|"catalogue"};
...
const AUTO_AIRPORT_RADIUS_KM=4;
...
async function airportSuggestions(userId:number,candidates:Array<{lat:number;lon:number}>):Promise<Array<AirportCandidate&{score:number}>>{
  ...
  const ranked=new Map<string,AirportCandidate&{score:number}>(),add=(item:AirportCandidate&{score:number})=>{const previous=ranked.get(item.ident);if(!previous||item.source==="custom"&&previous.source!=="custom"||item.source===previous.source&&item.score<previous.score)ranked.set(item.ident,item)};
  ...
  for(const airport of nearestCatalogAirports(valid,AIRPORT_CANDIDATE_RADIUS_KM,12))add({ident:airport.ident,name:airport.name,distanceKm:airport.distanceKm,score:airport.score,confidence:airportConfidence(airport.distanceKm),source:"catalogue"});
...
function automaticAirport(candidates:Array<AirportCandidate&{score:number}>):AirportDetection{
  const best=selectAutomaticAirport(candidates);if(!best)return null;
...
export async function redetectFlightAirports(flightId:number,_:FlightActionState,_form:FormData):Promise<FlightActionState>{
  const {userId}=await requireUser();await ensureDatabaseOptimizations();if(!Number.isSafeInteger(flightId)||flightId<=0)return{error:"Invalid flight."};
  const lock=await sql`SELECT locked_at FROM flights WHERE id=${flightId} AND user_id=${userId} LIMIT 1` as Array<{locked_at:string|null}>;if(!lock[0])return{error:"Flight not found."};if(lock[0].locked_at)return{error:"Unlock the flight before changing detected airports."};
  ...
  if(!parts.length)return{error:"Flight has no usable GPS track."};
  ...
  if(!departure&&!arrival)return{error:`No airport could be selected automatically within ${AUTO_AIRPORT_RADIUS_KM} km of the track endpoints.`};
  ...
  const found=[departure?`departure ${departure.ident} (${departure.distanceKm} km)`:"",arrival?`arrival ${arrival.ident} (${arrival.distanceKm} km)`:""].filter(Boolean).join(" · ");return{success:`Saved: ${found}.`};
}

export async function createFlight(_: FlightActionState, form: FormData): Promise<FlightActionState> {
  const { userId } = await requireUser(); await ensureDatabaseOptimizations();await Promise.all([ensureV159Schema(),ensureV162Schema(),ensureV164Schema()]); const parsed = parseFlightInput(form),expenseResult=parseFlightExpenses(form);
  ...
  const results=await sql.transaction([
    ...
    sql`WITH inserted AS (
      INSERT INTO flights (user_id,date,evidence,registration,aircraft_type,aircraft_class,regulatory_category,balloon_class,balloon_group,launch_method,launches,departure,arrival,off_block,takeoff,landing,on_block,starts,commander,instructor,role,task,purpose_code,price_per_hour,billing_basis,note,operation_type,engine_type,landings_day,landings_night,movement_evidence_recorded,takeoffs_day,takeoffs_night,approaches_day,approaches_night,night_minutes,ifr_minutes,pic_minutes,copilot_minutes,dual_minutes,instructor_minutes,verification_name,verification_reference)
      SELECT ${userId},${f.date},${f.evidence},${f.registration},${f.aircraftType},${f.aircraftClass},${f.regulatoryCategory},${f.balloonClass},${f.balloonGroup},${f.launchMethod},${f.launches},${departure},${arrival},${f.offBlock},${f.takeoff},${f.landing},${f.onBlock},${f.starts},${f.commander},${f.instructor},${f.role},${f.task},${f.purposeCode},${price},${f.billingBasis},${f.note},${f.operationType},${f.engineType},${f.landingsDay},${f.landingsNight},${f.movementEvidenceRecorded},${f.takeoffsDay},${f.takeoffsNight},${f.approachesDay},${f.approachesNight},${f.nightMinutes},${f.ifrMinutes},${f.picMinutes},${f.copilotMinutes},${f.dualMinutes},${f.instructorMinutes},${f.verificationName},${f.verificationReference}
...
export async function importKmlFlight(_:FlightActionState,form:FormData):Promise<FlightActionState>{
  const {userId}=await requireUser();await ensureDatabaseOptimizations();await Promise.all([ensureV162Schema(),ensureV164Schema()]);const file=form.get("kml");if(!(file instanceof File)||!file.size)return {error:"Select a KML, GPX or CSV file."};if(file.size>8*1024*1024)return {error:"File exceeds 8 MB."};
  let points;try{points=parseTrackFile(await file.text(),file.name)}catch{return {error:"File could not be read."}}if(points.length<2)return {error:"No usable GPS track was found."};
  const registration=String(form.get("registration")??"").trim().toUpperCase();if(!registration)return {error:"Select an aircraft registration."};const evidence=String(form.get("evidence")||"ULL"),role=String(form.get("role")||"PIC"),aircraftType=String(form.get("aircraftType")??""),aircraftClass=String(form.get("aircraftClass")||"ULL"),task=String(form.get("task")??"GPS import"),billing=serializeBilling(form.get("billingBasis"),form.get("billingShare"));
  const contextRows=await sql`SELECT COALESCE(regulatory_category,'') regulatory_category,COALESCE(balloon_class,'') balloon_class,COALESCE(balloon_group,'') balloon_group FROM aircraft WHERE user_id=${userId} AND UPPER(TRIM(registration))=${registration} LIMIT 1` as Array<{regulatory_category:string;balloon_class:string;balloon_group:string}>;const regulatoryCategory=regulatoryAircraftCategory({regulatoryCategory:contextRows[0]?.regulatory_category,aircraftClass,evidence}),balloonClass=regulatoryCategory==="BALLOON"?String(contextRows[0]?.balloon_class||"").trim().toUpperCase():"",balloonGroup=balloonClass==="HOT_AIR_BALLOON"?String(contextRows[0]?.balloon_group||"").trim().toUpperCase():"";
  if(regulatoryCategory==="BALLOON"&&!balloonClass)return{error:"The selected balloon aircraft profile is missing its Part-BFCL balloon class. Update the aircraft profile before importing this track."};if(balloonClass==="HOT_AIR_BALLOON"&&!balloonGroup)return{error:"The selected hot-air balloon profile is missing group A, B, C or D. Update the aircraft profile before importing this track."};
  const rawIndices=String(form.get("splitIndices")||"").split(",").filter(Boolean).map(Number),indices=[...new Set(rawIndices)].filter(value=>Number.isSafeInteger(value)&&value>0&&value<points.length-1).sort((a,b)=>a-b);if(indices.length!==rawIndices.length)return{error:"Invalid split. Upload the file again."};
  const parts=splitPoints(points,indices),partCount=Number(form.get("partCount"));if(!Number.isSafeInteger(partCount)||partCount!==parts.length||partCount<1||partCount>20)return{error:"Reviewed flight count does not match the track split."};const groundOnly=parts.findIndex(part=>!hasAirborneMovement(part));if(groundOnly>=0)return{error:`Section ${groundOnly+1} contains no credible flight movement. Adjust or remove the split.`};
  ...
  const validTime=(value:string)=>!value||/^([01]\d|2[0-3]):[0-5]\d$/.test(value);for(let index=0;index<parts.length;index++){if(String(form.get(`part_${index}_reviewed`)||"")!=="yes")return{error:`Flight ${index+1} was not reviewed.`};const date=String(form.get(`part_${index}_date`)||""),offBlock=String(form.get(`part_${index}_offBlock`)||""),takeoff=String(form.get(`part_${index}_takeoff`)||""),landing=String(form.get(`part_${index}_landing`)||""),onBlock=String(form.get(`part_${index}_onBlock`)||"");if(flightDateKey(date)!==date||[offBlock,takeoff,landing,onBlock].some(value=>!validTime(value)))return{error:`Flight ${index+1} has an invalid date or time.`};reviewed.push({date,offBlock,takeoff,landing,onBlock,departure:canonicalAirportIdent(String(form.get(`part_${index}_departure`)||"").slice(0,16)),arrival:canonicalAirportIdent(String(form.get(`part_${index}_arrival`)||"").slice(0,16)),starts:Math.max(0,Math.min(99,Number(form.get(`part_${index}_starts`)||1))),note:String(form.get(`part_${index}_note`)||"").trim().slice(0,2000)})}
  const priceCache=new Map<string,number|null>();const prepared=[];for(let index=0;index<parts.length;index++){const part=parts[index],stats=trackStats(part),envelope=flightEnvelope(part),values=reviewed[index];let price=priceCache.get(values.date);if(!priceCache.has(values.date)){price=await resolvedPrice(userId,registration,values.date);priceCache.set(values.date,price??null)}const [detectedDeparture,detectedArrival]=await Promise.all([values.departure?Promise.resolve(null):nearestAirport(userId,envelope.departureCandidates),values.arrival?Promise.resolve(null):nearestAirport(userId,envelope.arrivalCandidates)]),departure=values.departure||detectedDeparture?.ident||"",arrival=values.arrival||detectedArrival?.ident||"",partNote=parts.length>1?`${values.note}${values.note?' · ':''}Reviewed import ${index+1}/${parts.length}`:values.note,suffix=splitTrackFileName(file.name,index,parts.length),allocation=allocatedFunctionTimes(role,flightMinutes(values.offBlock,values.onBlock));prepared.push({part,stats,values,price:price??null,departure,arrival,partNote,suffix,allocation,fingerprint:flightFingerprint(userId,{date:values.date,registration,offBlock:values.offBlock,departure,arrival})})}
  if(new Set(prepared.map(item=>item.fingerprint)).size!==prepared.length)return{error:"Two imported sections have the same date, time and route. Adjust the split or times."};
  ...
  if(existing.some(rows=>rows.length))return{error:"At least one reviewed flight already exists. The duplicate import was blocked."};
  let lastId=0;try{const fingerprints=[...new Set(prepared.map(item=>item.fingerprint))].sort(),locks=fingerprints.map(value=>sql`SELECT pg_advisory_xact_lock(hashtextextended(${value},0))`),inserts=prepared.map(item=>sql`WITH inserted AS (INSERT INTO flights(user_id,date,evidence,registration,aircraft_type,aircraft_class,regulatory_category,balloon_class,balloon_group,launch_method,launches,departure,arrival,off_block,takeoff,landing,on_block,starts,commander,instructor,role,task,price_per_hour,billing_basis,note,operation_type,engine_type,landings_day,landings_night,pic_minutes,copilot_minutes,dual_minutes,instructor_minutes) SELECT ${userId},${item.values.date},${evidence},${registration},${aircraftType},${aircraftClass},${regulatoryCategory},${balloonClass},${balloonGroup},'',0,${item.departure},${item.arrival},${item.values.offBlock},${item.values.takeoff},${item.values.landing},${item.values.onBlock},${item.values.starts},'', '',${role},${task},${item.price},${billing},${item.partNote},'SP',${defaultEngineType(aircraftClass)},${item.values.starts},0,${item.allocation.picMinutes},${item.allocation.copilotMinutes},${item.allocation.dualMinutes},${item.allocation.instructorMinutes} WHERE NOT EXISTS(SELECT 1 FROM flights WHERE user_id=${userId} AND date::text=${item.values.date} AND UPPER(TRIM(registration))=${registration} AND COALESCE(off_block,'')=${item.values.offBlock} AND UPPER(TRIM(COALESCE(departure,'')))=${item.departure} AND UPPER(TRIM(COALESCE(arrival,'')))=${item.arrival}) RETURNING id) INSERT INTO flight_tracks(user_id,flight_id,file_name,imported_at,point_count,distance_km,start_utc,end_utc,min_alt_m,max_alt_m,coordinates_json,overview_coordinates_json,overview_version) SELECT ${userId},(SELECT id FROM inserted),${item.suffix},NOW(),${item.stats.pointCount},${item.stats.distanceKm},${item.stats.startUtc},${item.stats.endUtc},${item.stats.minAlt},${item.stats.maxAlt},${JSON.stringify(item.part)},${JSON.stringify(overview(item.part))},1 RETURNING flight_id`),results=await sql.transaction([...locks,...inserts]);const lastRows=results.at(-1) as Array<{flight_id:number|string}>;lastId=Number(lastRows?.[0]?.flight_id);if(!lastId)throw new Error("Import did not return a flight id")}catch(error){console.error("reviewed-import-transaction-failed",error);return{error:"Import failed and the transaction was rolled back. No partial flights were created."}}
...
export async function updateFlight(id: number, _: FlightActionState, form: FormData): Promise<FlightActionState> {
  const { userId } = await requireUser(); await ensureDatabaseOptimizations();await Promise.all([ensureV159Schema(),ensureV162Schema(),ensureV164Schema()]); if (!Number.isSafeInteger(id) || id <= 0) return { error: "Invalid record." };
...
export async function attachKmlTrack(flightId:number,_:FlightActionState,form:FormData):Promise<FlightActionState>{
  const {userId}=await requireUser();await ensureDatabaseOptimizations();if(!Number.isSafeInteger(flightId)||flightId<=0)return{error:"Invalid flight."};
  const owned=await sql`SELECT id,locked_at FROM flights WHERE id=${flightId} AND user_id=${userId}` as Array<{id:number;locked_at:string|null}>;if(!owned[0])return{error:"Flight not found."};if(owned[0].locked_at)return{error:"Unlock the flight before changing its GPS track."};
  const file=form.get("kml");if(!(file instanceof File)||!file.size)return{error:"Select a KML, GPX or CSV file."};if(file.size>8*1024*1024)return{error:"File exceeds 8 MB."};
  let points;try{points=parseTrackFile(await file.text(),file.name)}catch{return{error:"File could not be read."}}if(points.length<2)return{error:"File contains no usable track."};
...
export async function setFlightLock(flightId:number,form:FormData){
  const {userId}=await requireUser();await ensureDatabaseOptimizations();if(!Number.isSafeInteger(flightId)||flightId<=0)return;
  const lock=String(form.get("lock")||"")==="yes";
  if(lock)await sql`UPDATE flights SET locked_at=NOW(),locked_by_user_id=${userId} WHERE id=${flightId} AND user_id=${userId} AND locked_at IS NULL`;
  else await sql`UPDATE flights SET locked_at=NULL,locked_by_user_id=NULL WHERE id=${flightId} AND user_id=${userId} AND locked_at IS NOT NULL`;
  revalidatePath(`/flights/${flightId}`);revalidatePath("/flights");revalidatePath("/database");
}
