...
export async function importKmlFlight(_:FlightActionState,form:FormData):Promise<FlightActionState>{
  ...
  for(let index=0;index<parts.length;index++){const part=parts[index],stats=trackStats(part),start=localParts(stats.startUtc),end=localParts(stats.endUtc),manualDate=String(form.get("date")??""),date=manualDate||start?.date||new Date().toISOString().slice(0,10),offBlock=String(form.get("offBlock")||start?.time||""),onBlock=String(form.get("onBlock")||end?.time||""),price=await resolvedPrice(userId,registration,date),partNote=parts.length>1?`${note}${note?' · ':''}Smart KML ${index+1}/${parts.length}`:note,departure=manualDeparture||await nearestOwnAirport(userId,part[0]),arrival=manualArrival||await nearestOwnAirport(userId,part.at(-1)!);const detectedLandings=landingCount(part),starts=String(form.get("splitMode"))==="auto"?detectedLandings:Math.max(0,Number(form.get("starts")||detectedLandings));
