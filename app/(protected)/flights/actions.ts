...
export async function redetectFlightAirports(flightId:number,_:FlightActionState,_form:FormData):Promise<FlightActionState>{
  ...
  if(!departure&&!arrival)return{error:`No airport was found within ${AUTO_AIRPORT_RADIUS_KM} km of the track endpoints.`};
...
export async function importKmlFlight(_:FlightActionState,form:FormData):Promise<FlightActionState>{
  ...
  const priceCache=new Map<string,number|null>();const prepared=[];for(let index=0;index<parts.length;index++){const part=parts[index],stats=trackStats(part),envelope=flightEnvelope(part),values=reviewed[index];let price=priceCache.get(values.date);if(!priceCache.has(values.date)){price=await resolvedPrice(userId,registration,values.date);priceCache.set(values.date,price??null)}const [detectedDeparture,detectedArrival]=await Promise.all([values.departure?Promise.resolve(null):nearestAirport(userId,envelope.departureCandidates),values.arrival?Promise.resolve(null):nearestAirport(userId,envelope.arrivalCandidates)]),departure=values.departure||detectedDeparture?.ident||"",arrival=values.arrival||detectedArrival?.ident||"",partNote=parts.length>1?`${values.note}${values.note?' · ':''}Reviewed import ${index+1}/${parts.length}`:values.note,suffix=parts.length>1?`__part${index+1}-of-${parts.length}.kml`:file.name,allocation=allocatedFunctionTimes(role,flightMinutes(values.offBlock,values.onBlock));prepared.push({part,stats,values,price:price??null,departure,arrival,partNote,suffix:suffix.slice(0,240),allocation,fingerprint:flightFingerprint(userId,{date:values.date,registration,offBlock:values.offBlock,departure,arrival})})}
