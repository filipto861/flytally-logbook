...
export async function createFlight(_: FlightActionState, form: FormData): Promise<FlightActionState> {
  const { userId } = await requireUser(); await ensureDatabaseOptimizations();await Promise.all([ensureV159Schema(),ensureV162Schema()]); const parsed = parseFlightInput(form),expenseResult=parseFlightExpenses(form);
...
export async function importKmlFlight(_:FlightActionState,form:FormData):Promise<FlightActionState>{
  const {userId}=await requireUser();await ensureDatabaseOptimizations();await ensureV162Schema();const file=form.get("kml");if(!(file instanceof File)||!file.size)return {error:"Select a KML, GPX or CSV file."};if(file.size>8*1024*1024)return {error:"File exceeds 8 MB."};
  ...
  const contextRows=await sql`SELECT COALESCE(regulatory_category,'') regulatory_category FROM aircraft WHERE user_id=${userId} AND UPPER(TRIM(registration))=${registration} LIMIT 1` as Array<{regulatory_category:string}>;const regulatoryCategory=regulatoryAircraftCategory({regulatoryCategory:contextRows[0]?.regulatory_category,aircraftClass,evidence});
...
export async function updateFlight(id: number, _: FlightActionState, form: FormData): Promise<FlightActionState> {
  const { userId } = await requireUser(); await ensureDatabaseOptimizations();await Promise.all([ensureV159Schema(),ensureV162Schema()]); if (!Number.isSafeInteger(id) || id <= 0) return { error: "Invalid record." };
