...
export async function GET(request:NextRequest){
  if(!cronRequestAuthorized(process.env.CRON_SECRET,request.headers.get("authorization")))return NextResponse.json({error:"Unauthorized"},{status:401});
  await Promise.all([ensureV1353Schema(),ensureV151Schema()]);
