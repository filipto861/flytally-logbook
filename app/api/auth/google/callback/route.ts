...
const errorRedirect=(request:NextRequest,code:string)=>NextResponse.redirect(new URL(`/login?error=${code}`,request.url));
...
export async function GET(request:NextRequest){
  ...
  try{
    ...
    if(!subject||!email||claims?.email_verified!==true)return errorRedirect(request,"google_unverified");
    ...
    if(!rows[0]){
      if(!flow.invite)return errorRedirect(request,"invite_required");
      ...
      if(existing[0])return errorRedirect(request,"google_link_required");
      ...
      if(!rows[0])return errorRedirect(request,"invite_invalid");
    }
    const user=rows[0];if(Number(user.active)!==1)return errorRedirect(request,"account_inactive");
    ...
    const response=NextResponse.redirect(new URL("/dashboard",request.url));response.cookies.delete(GOOGLE_FLOW_COOKIE);return response;
  }catch(error){console.error("google-auth-failed",error);return errorRedirect(request,"google_failed");}
}
