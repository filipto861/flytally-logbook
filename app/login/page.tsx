...
export default async function LoginPage({searchParams}:{searchParams:Promise<{error?:string;reset?:string}>}) {
  ...
  const params=await searchParams,error=messages[String(params.error||"")],success=params.reset==="success"?"Password changed. Sign in with your new password.":undefined;
  return (
    <main className="login-shell">
      <section className="login-card">
        ...
        <LoginForm google={googleConfigured()} externalError={error} success={success}/>
      </section>
    </main>
  );
}
